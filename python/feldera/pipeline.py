...
metrics"""
    ...
    def delete(self, unbind_storage: bool = False):
        """
        Deletes the pipeline.

        The pipeline must be stopped, and the storage unbinded before it can be
        deleted.

        :param unbind_storage: True if the storage should be unbinded before
...
pipeline '{self.name}' to sync checkpoint '{uuid}'"""
    ...
    def unbind_storage(self):
        """
        Unbinds the storage of the pipeline if it is currently bound.
        This action cannot be canceled, and will delete all the pipeline
        storage.
        """

        if self.storage_status() == StorageStatus.BOUND:
            self.client.unbind_storage(self.name)
