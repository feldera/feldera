...
{resp.deployment_error.get("message", "")}"""
    ...
    def commit_transaction(self, pipeline_name: str, transaction_id: Optional[int] = None, wait: bool = True, timeout_s: Optional[float] = None):
        """
        ...
        if current_transaction_id == 0:
            raise RuntimeError("Attempting to commit a transaction, but there is no transaction in progress")

        if transaction_id and current_transaction_id != transaction_id:
            raise ValueError(f"Specified transaction id {transaction_id} doesn't match current active transaction id {current_transaction_id}")
        ...
        while True:
            if timeout_s is not None:
                ...
                if elapsed > timeout_s:
                    raise TimeoutError(
                        f"Timed out waiting for transaction to commit"
                    )
            ...
            logging.debug(
                "commit hasn't completed, waiting for 1 more second"
            )
            time.sleep(1.0)
