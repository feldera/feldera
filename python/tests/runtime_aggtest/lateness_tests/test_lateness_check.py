...
class lateness_tumble_tbl_v(TstView):
    def __init__(self):
        self.sql = """CREATE MATERIALIZED VIEW tumble_tbl_v
                    WITH ('emit_final' = 'window_start')
                    AS
                    SELECT
                        window_start,
...
class lateness_rolling_sum_range_legal(TstView):
    def __init__(self):
        self.sql = """CREATE MATERIALIZED VIEW rolling_sum_range_legal
                    AS
                    SELECT
                        ts,
                        SUM(value) OVER (
