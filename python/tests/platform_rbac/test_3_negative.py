...
#
# A token is admitted if any trust in the tenant matches it, and several of the
# patterns below are permissive enough to swallow other cases' subjects: `a*b`
# takes anything starting `a` and ending `b`, `repo:**` anything starting
# `repo:`. So each case prefixes its pattern and subject with its own name,
# giving it a subject no other case can match. The prefix is literal and
# identical on both sides, so it cannot change what is being measured.
CLAIM_PATTERNS = [
    ...
    ("prefix-match", "repo:acme/*", "repo:acme/api", True),
    ("extra-before-slash", "repo:acme/*", "repo:acmex/api", False),
    ("missing-separator", "repo:acme/*", "repo:acme", False),
    ("start-is-anchored", "repo:acme/*", "xrepo:acme/api", False),
    ("wrong-case", "repo:acme/*", "repo:ACME/api", False),
]
...
def test_deleting_a_trust_revokes_its_access(
    api: Api, primary_idp: Issuer, workload_idp: Issuer
):
    """Access granted by a trust ends when the trust does, and at once.

    Granting is only half of a trust's life. Nothing else here proves the other
    half, and a stale grant that outlives its record is the failure that matters:
    an operator who revokes a workload has no other lever to pull.
    """
    admin = primary_idp.token("admin", email="admin@example.com", tenants=[TENANT])
    subject = "revocable-bot"
    register_pattern_trust(
        api, admin, workload_idp, "revocable", subject, DEFAULT_AUDIENCE
    )

    token = workload_idp.token(subject)
    assert api.status("GET", "/pipelines", token=token, tenant=TENANT) == 200

    assert (
        api.status("DELETE", "/oidc_trust/revocable", token=admin, tenant=TENANT) == 200
    )

    # The same token, unchanged and still unexpired, is now refused. No cache
    # may keep it alive: the trust is consulted per request.
    assert api.status("GET", "/pipelines", token=token, tenant=TENANT) in (401, 403)
    # And the trust is gone from the tenant's list, not merely unmatched.
    assert api.status("GET", "/oidc_trust/revocable", token=admin, tenant=TENANT) == 404


def test_deleting_a_trust_leaves_the_others(
    api: Api, primary_idp: Issuer, workload_idp: Issuer
):
    """Revoking one workload does not revoke its neighbours.

    Both trusts name the same issuer, so this also covers the case where the
    deleted one was the only reason to think that issuer interesting.
    """
    admin = primary_idp.token("admin", email="admin@example.com", tenants=[TENANT])
    for name in ("neighbour-kept", "neighbour-dropped"):
        register_pattern_trust(
            api, admin, workload_idp, name, f"{name}-bot", DEFAULT_AUDIENCE
        )

    kept = workload_idp.token("neighbour-kept-bot")
    dropped = workload_idp.token("neighbour-dropped-bot")
    assert api.status("GET", "/pipelines", token=kept, tenant=TENANT) == 200
    assert api.status("GET", "/pipelines", token=dropped, tenant=TENANT) == 200

    assert (
        api.status(
            "DELETE", "/oidc_trust/neighbour-dropped", token=admin, tenant=TENANT
        )
        == 200
    )

    assert api.status("GET", "/pipelines", token=dropped, tenant=TENANT) in (401, 403)
    assert api.status("GET", "/pipelines", token=kept, tenant=TENANT) == 200


def test_deleting_an_absent_trust_is_not_found(api: Api, primary_idp: Issuer):
    admin = primary_idp.token("admin", email="admin@example.com", tenants=[TENANT])
    assert (
        api.status("DELETE", "/oidc_trust/never-existed", token=admin, tenant=TENANT)
        == 404
    )
