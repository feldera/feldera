...
class aggtst_varchar_array_agg_value(TstView):
    def __init__(self):
        ...
        self.data = [{'c1': [None, '@abc', 'hello', 'hello'], 'c2': ['abc   d', 'variable-length', 'exampl e', 'fred']}]
...
class aggtst_varchar_array_agg_gby(TstView):
    def __init__(self):
        # Validated on Postgres
        self.data = [{'id': 0, 'c1': [None, 'hello'], 'c2': ['abc   d', 'fred']},
...
class aggtst_varchar_array_agg_distinct(TstView):
    def __init__(self):
        # Validated on Postgres
        self.data = [{'c1': [None, '@abc', 'hello'], 'c2': ['abc   d', 'exampl e', 'fred', 'variable-length']}]
...
class aggtst_varchar_array_agg_where(TstView):
    def __init__(self):
        # Validated on Postgres
        self.data = [{'f_c1': [None, '@abc', 'hello'], 'f_c2': ['abc   d', 'variable-length', 'exampl e']}]
...
class aggtst_varchar_array_agg_where_gby(TstView):
    def __init__(self):
        # Validated on Postgres
        self.data = [{'id': 0, 'f_c1': [None], 'f_c2': ['abc   d']},
                     {'id': 1, 'f_c1': ['@abc', 'hello'], 'f_c2': ['variable-length', 'exampl e']}]
