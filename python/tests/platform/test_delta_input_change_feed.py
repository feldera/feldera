...
def test_delta_input_change_feed_merge(pipeline_name):
    """Replaying a `MERGE` commit through the change feed reads only the rows the
    commit recorded, and lands on the same contents as reading it through file
    actions.

    The merge updates one row, deletes another and inserts a third, so it records
    all four change types. Replaying v5 alone isolates it: the snapshot at v4 is
    the table before the merge, so every row difference is the merge's doing.
...
def test_delta_input_change_feed_reads_only_changed_rows(pipeline_name):
    ...
    retractions cancel the insertions back to the same table. Both modes reach
    the same contents, so the ingested record count is what tells them apart,
...
def test_delta_input_change_feed_partition_column(pipeline_name):
    """A partitioned table's partition column arrives with the right value,
    both through the change feed and through the file-action fallback."""
