...
"""
    ...
    assert diff["program_diff"] == {'added_tables': [], 'added_views': ['v2'], 'modified_tables': [], 'modified_views': [], 'removed_tables': [], 'removed_views': []}
    ...
    sql_with_new_table = original_sql + """CREATE TABLE t2(y int) WITH ('materialized'='true');
CREATE MATERIALIZED VIEW v3 AS SELECT MAX(y) AS m FROM t2;
    ...
    diff = json.loads(diff)
    print(f"Pipeline diff: {diff}")
    assert diff["program_diff"] == {'added_tables': ['t2'], 'added_views': ['v3'], 'modified_tables': [], 'modified_views': [], 'removed_tables': [], 'removed_views': []}
    ...
    sql_with_new_table = original_sql + """CREATE TABLE t2(y int, s string) WITH ('materialized'='true');
CREATE MATERIALIZED VIEW v3 AS SELECT MAX(y) AS m FROM t2;
    """
    pipeline.modify(sql=sql_with_new_table)
    ...
    diff = json.loads(diff)
    print(f"Pipeline diff: {diff}")
    assert diff["program_diff"] == {'added_tables': [], 'added_views': [], 'modified_tables': ['t2'], 'modified_views': ['v3'], 'removed_tables': [], 'removed_views': []}
    ...
    diff = json.loads(diff)
    print(f"Pipeline diff: {diff}")
    assert diff["program_diff"] == {'added_tables': [], 'added_views': [], 'modified_tables': [], 'modified_views': [], 'removed_tables': [], 'removed_views': ['v3']}
    ...
    # The table hasn't change, so the previous 3 rows should still be there.
    ...
    diff = json.loads(diff)
    print(f"Pipeline diff: {diff}")
    assert diff["program_diff"] == {'added_tables': [], 'added_views': [], 'modified_tables': [], 'modified_views': [], 'removed_views': [], 'removed_tables': ['t2']}
...
@enterprise_only
@gen_pipeline_name
def test_bootstrap_lateness_enterprise(pipeline_name):
    """
    Enterprise: bootstrapping a program with lateness must fail.
    """

    sql = """CREATE TABLE t1(x int lateness 0);
CREATE MATERIALIZED VIEW v1 AS SELECT COUNT(*) AS c FROM t1;
"""
