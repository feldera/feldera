...
class TestAggregatesBase(unittest.TestCase):
    def setUp(self) -> None:
        self.data = [{"insert":{"id": 0, "c1": 1, "c2": 2, "c3": 3, "c4": 4, "c5": 5, "c6": 6, "c7": None, "c8": 8}},
                       {"insert":{"id": 0,"c1": None, "c2": 20, "c3": 30, "c4": 40, "c5": None, "c6": 60, "c7": 70, "c8": 80}},
                        {"insert":{"id": 1, "c1": 11, "c2": 22, "c3": None, "c4": 45, "c5": 51, "c6": 61, "c7": 72, "c8": 88}}]       
        return super().setUp()
...
class Sum_Groupby(TestAggregatesBase):
    def test_sum_groupby(self):
        ...
        expected_data = [{"id": 0, "c1": 1, "c2": 22, "c3": 33, "c4": 44, "c5": 5, "c6": 66, "c7": 70, "c8": 88},
                         {"id": 1, "c1": 34, "c2": 48, "c3": 3, "c4": 54, "c5": 54, "c6": 81, "c7": 115, "c8": 91}]
...
class Sum_Where_Groupby(TestAggregatesBase):
    ...
    def test_sum_where_groupby1(self):
        ...
        # validated using postgres
        expected_data = [{"id": 1, "sum_c1": 11, "sum_c2": 22, "sum_c3":None, "sum_c4": 45, "sum_c5": 51, "sum_c6": 61, "sum_c7": 72, "sum_c8": 88}]
...
class Sum_Fail0(TestAggregatesBase):
    # checks for assertion fail on incorrect expected output
    def test_sum_fail0(self):
        pipeline_name ="test_sum_fail0"
        expected_data = [{"c1": 0, "c2": 0, "c3" : 0, "c4" : 4, "c5" : 5, "c6" : 6,	"c7":7, "c8": 8}] 
        table_name = "sum_fail0"
        view_query = f'''CREATE VIEW sum_view AS SELECT
                            SUM(c1) AS c1, SUM(c2) AS c2, SUM(c3) AS c3, SUM(c4) AS c4, SUM(c5) AS c5, SUM(c6) AS c6, SUM(c7) AS c7, SUM(c8) AS c8
                         FROM {table_name}'''  
