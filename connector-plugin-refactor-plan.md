...
**Re-export-chain cleanup (PR 7g, commit 2)**: PR 2 moves `IntegratedOutputEndpoint` to `feldera-adapterlib` but leaves a two-hop re-export chain (`feldera_adapterlib::transport → adapters::integrated → adapters::lib`). PR 7f, when it adds the descriptor-build closures inside `integrated.rs`, lifts a `pub use feldera_adapterlib::transport::IntegratedOutputEndpoint;` into that module to bring the trait into scope for function signatures — meaning by the time PR 7g runs, both ends of the chain still exist. Commit 2's job: (a) make `adapters::lib.rs` import directly from `feldera_adapterlib::transport` (skipping the integrated module), and (b) downgrade `integrated.rs`'s `pub use` to a private `use` (verify with `rg 'integrated::IntegratedOutputEndpoint'` that no external caller depends on the re-export). Same applies to `Encoder`/`OutputEndpoint` imports in `integrated.rs` (no longer needed there once the factory bodies are trivial registry calls). **Audit conclusion (do not remove)**: `pub use crate::integrated::postgres::PostgresOutputEndpoint` and analogous `delta_table` re-exports remain load-bearing because criterion benches import the concrete types directly. Removing them is a PR 7h candidate, gated on migrating those benches to registry dispatch.
