...
- [ ] **PR 7g — final cleanup** (one PR, three commits): with all bundled connectors registry-driven after 7f, strip the post-migration leftovers. Each commit is independently small and reviewable, but they share the "PR 7 swept the connectors, now sweep the scaffolding" theme. Splitting into three separate top-level PRs would be needless review overhead.
  - **Commit 1 — strip dead match arms**: most of this work has already happened during PRs 7c (output collapsed after kafka in 7d) and 7e (input collapsed after nexmark) — the `match config { _ => Ok(None) }` shells were left in place to keep diffs reviewable. This commit deletes the now-empty `match` shells in `input_transport_config_to_endpoint` and `output_transport_config_to_endpoint` (returning `Ok(None)` directly), and does the analogous strip for the two integrated factories after PR 7f. Also remove `#[allow(unused_variables)]` from `input_transport_config_to_endpoint` — with the match gone, every parameter is either consumed by the registry path or by the `Ok(None)` return.
