...
- [ ] **PR 7g — final cleanup** (one PR, three commits): with all bundled connectors registry-driven after 7f, strip the post-migration leftovers. Each commit is independently small and reviewable, but they share the "PR 7 swept the connectors, now sweep the scaffolding" theme. Splitting into three separate top-level PRs would be needless review overhead.
  ...
  - **Commit 2 — re-export-chain cleanup**: with `adapters::integrated` no longer the dispatch home, switch `adapters::lib.rs` to import `IntegratedOutputEndpoint` directly from `feldera_adapterlib::transport` (collapsing the two-hop chain established by PR 2). Drop `Encoder` / `OutputEndpoint` imports from `integrated.rs` if no longer referenced. Audit whether `pub use crate::integrated::postgres::PostgresOutputEndpoint` (and analogous re-exports for `delta_table`/`iceberg`) is still needed after the fallback match in `integrated.rs` is gone — if the only consumer was the deleted match arm, the re-export goes too.
