...
def make_config(project):
    config = DBSPPipelineConfig(project, 8, "GreenTrip Pipeline")
    config.add_kafka_input(
        name="Green Trips Data",
        stream="GREEN_TRIPDATA",
        config=KafkaInputConfig.from_dict(
            {"topics": ["green_trip_demo_large_input"], "auto.offset.reset": "earliest"}
        ),
        format=CsvInputFormatConfig(),
    )
    config.add_kafka_output(
        name="Online Feature Calculation",
        stream="FEATURES",
        config=KafkaOutputConfig.from_dict({"topic": "green_trip_demo_large_output"}),
