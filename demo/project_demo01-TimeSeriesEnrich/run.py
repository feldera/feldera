...
def prepare_feldera(api_url, start_pipeline):
    ...
    for (connector_name, stream, topic_topics, is_input) in [
        ("time-series-enrich-demographics-large", 'DEMOGRAPHICS', ['fraud_demo_large_demographics'], True),
        ("time-series-enrich-transactions-large", 'TRANSACTIONS', ['fraud_demo_large_transactions'], True),
        ("time-series-enrich-join-demographics-with-transactions",
         'TRANSACTIONS_WITH_DEMOGRAPHICS', "fraud_demo_large_enriched", False),
    ]:
        # Create connector
