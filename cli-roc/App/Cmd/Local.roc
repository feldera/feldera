interface App.Cmd.Local
    ...
    imports [
      pf.Task.{ Task, await }, pf.Dir,  pf.Stdout, pf.Path, pf.File, pf.Http, pf.Cmd,
      json.Core.{jsonWithOptions},
      App.Cmd.Workspace.{workspaceCtx, updateWorkspaceDefault, onConflictReplace},
...
skipLatter: ({}a -> Task {} *) -> Task {} *
skipLatter = \ _task ->
  Stdout.line "Skipping latter actions!"

# skipLatterWhen = \ task pred ->
#   task |> await \ v -> if pred v then Task.ok {} else _task
...
cmdLocal =
  ...
  {} <- when x is
    NotDetected -> \ t ->
      {} <- Stdout.line "NotDetected!" |> await
      skipLatter t
    Installed -> \ t ->
      {} <- Stdout.line "Installed!" |> await
      t {}
  {} <- Stdout.line "Checking Feldera deployment..." |> await
  validDeployment <- pingFelderaWorkaround "http://localhost:8080" |> await
  {} <- (if validDeployment
    then
      Task.ok {}
    else
      ...
      if exists
        then Task.ok {}
        else downloadFelderaImage
  ) |> await
  {} <- {name: "local", url: "http://localhost:8080"} |> updateWorkspaceDefault "." onConflictReplace |> await
...
startupLocalDeployment =
  ...
  {} <- Cmd.new "docker"
    ...
    |> (\ cmd -> if alreadyRun
      then cmd |> Cmd.arg "up"
      else cmd |> Cmd.arg "--profile" |> Cmd.arg "demo" |> Cmd.arg "up"
    )
    # |> Cmd.output
    # |> Task.onErr (\e -> crash (Inspect.toStr e))
    # |> await
...
cmdLocalUpgrade =
  {} <- Cmd.new "docker compose -f"
    |> Cmd.arg localComposeFile
    |> Cmd.arg "--profile demo down"
    |> Cmd.output
    |> Task.onErr (\e -> crash (Inspect.toStr e))
    |> await
  {} <- Cmd.new "docker compose -f"
    |> Cmd.arg localComposeFile
    |> Cmd.arg "--profile demo up --pull always --force-recreate -V"
    |> Cmd.output
    |> Task.onErr (\e -> crash (Inspect.toStr e))
    |> await
  Task.ok {}
