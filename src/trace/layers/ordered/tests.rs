...
use crate::trace::{
    layers::{
        column_leaf::OrderedColumnLeafBuilder,
...
fn empty_consumer() -> OrderedLayerConsumer<usize, usize, isize, usize> {
    OrderedLayerConsumer::from(
        UnorderedBuilder::<usize, OrderedColumnLeafBuilder<usize, isize>, usize>::new().done(),
    )
}
...
fn consumer_smoke_test() {
    ...
    let mut builder: UnorderedBuilder<usize, OrderedColumnLeafBuilder<usize, isize>, usize> =
...
fn remaining_values() {
    let mut builder =
        UnorderedBuilder::<usize, OrderedColumnLeafBuilder<usize, isize>, usize>::new();
    for idx in 0..10 {
