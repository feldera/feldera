//! Defines an operator that feeds data from the parent circuit to a nested circuit.
...
/// each parent timestamp).  The operator consumes a stream from the parent domain.
/// On each parent clock tick it reads and stores internally an element from the
/// stream.  On each nested clock tick, it outputs the current stored value and
/// updates it using a user-provided transformer function.
...
where
    ...
    T: Default,
{
    /// Creates a new `NestedSource` operator and binds a parent stream to its input.
    ///
    /// `input_stream` - A stream from the parent circuit.  The operator consumes a value from
    /// this stream on each parent clock tick.
    ///
    /// `circuit` - Child circuit where the operator is instantiated.
    ///
    /// `update` - Transformer function applied to the stored value on each nested clock tick.
    pub fn new(
