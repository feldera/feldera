...
fn main() {
    let hruntime = Runtime::run(1, |_runtime, _index| {
        ...
        let root = Root::build(move |circuit| {
            ...
            let p: Stream<_, ZSetHashMap<_, Weight>> = circuit.add_source(p_source);
            let q: Stream<_, ZSetHashMap<_, Weight>> = circuit.add_source(q_source);
            let r: Stream<_, ZSetHashMap<_, Weight>> = circuit.add_source(r_source);
            let c: Stream<_, ZSetHashMap<_, Weight>> = circuit.add_source(c_source);
            let u: Stream<_, ZSetHashMap<_, Weight>> = circuit.add_source(u_source);
            let s: Stream<_, ZSetHashMap<_, Weight>> = circuit.add_source(s_source);
            ...
            let (outp, outq) = circuit
                .iterate_with_conditions(|child| {
                    ...
                    let ir1 = p_by_2.join_incremental_nested(&p_by_1, |&_y, &x, &z| (x, z));
                    ir1.inspect(|zs: &ZSetHashMap<_, _>| println!("ir1: {}", zs.support_size()));

                    // IR2: q(x,r,z) := p(x,y), q(y,r,z)
                    let ir2 = p_by_2.join_incremental_nested(&q_by_1, |&_y, &x, &(r, z)| (x, r, z));
                    ...
                    let ir5 = q_by_2.join_incremental_nested(&s_by_1, |&_r, &(x, z), &q| (x, q, z));
