...
public class ReorderTemporalFiltersTests extends SqlIoTest {
    ...
    @Test
    public void onlyTheSharedTimestampIsHoisted() {
        DBSPCompiler compiler = this.testCompiler();
        ExpressionBuilder b = new ExpressionBuilder();
        DBSPVariablePath row = b.refVar(
                b.tup(b.i32(), DBSPTypeTimestamp.INSTANCE, DBSPTypeTimestamp.INSTANCE));

        // Field 1 is the shared timestamp; field 2 belongs to this view alone.
        DBSPExpression sharedLower = b.binary(DBSPOpcode.GT, b.field(row, 1), this.now());
        DBSPExpression sharedUpper = b.binary(DBSPOpcode.LT, b.field(row, 1), this.now());
        DBSPExpression privateBound = b.binary(DBSPOpcode.GT, b.field(row, 2), this.now());
        DBSPExpression plain = b.binary(DBSPOpcode.GT, b.field(row, 0), b.lit(2));

        // plain AND sharedLower AND privateBound AND sharedUpper
        DBSPExpression body = b.binary(DBSPOpcode.AND,
                b.binary(DBSPOpcode.AND,
                        b.binary(DBSPOpcode.AND, plain, sharedLower),
                        privateBound),
                sharedUpper);
        Assert.assertEquals(
                "((((((*p0).0) > 2) && (((*p0).1) > now())) && (((*p0).2) > now())) "
                        + "&& (((*p0).1) < now()))",
                this.print(compiler, body.closure(row)));

        DBSPExpression reordered = ReorderTemporalFilters.reorderAroundSharedTimestamp(
                body,
                conjunct -> {
                    if (conjunct == sharedLower || conjunct == sharedUpper) return "shared";
                    if (conjunct == privateBound) return "private";
                    return null;
                },
                sharedLower);
        Assert.assertEquals(
                "(((((*p0).1) > now()) && (((*p0).1) < now())) "
                        + "&& ((((*p0).0) > 2) && (((*p0).2) > now())))",
                this.print(compiler, reordered.closure(row)));
    }
    ...
    public void thresholdDecidesWhetherWindowsShare() {
        ...
        WindowInputStats alone = WindowInputStats.windows(this.getCC(twoViews(3)));
        Assert.assertEquals(2, alone.leftInputIds.size());
        Assert.assertEquals(2, alone.distinctInputCount());

        // 2 is over the threshold: share
        WindowInputStats shared = WindowInputStats.windows(this.getCC(twoViews(2)));
    ...
    @Test
    public void resultsSurviveSplittingTheTemporalConjuncts() {
        String sql = """
                SET feldera_window_sharing_threshold = 1;
                CREATE TABLE T(a INT, b INT, ts TIMESTAMP, ts2 TIMESTAMP);
                CREATE LOCAL VIEW V1 AS SELECT b FROM T
                WHERE ts >= NOW() - INTERVAL 100 YEARS AND a > 2 AND ts2 >= NOW() - INTERVAL 1 MONTHS;
                CREATE LOCAL VIEW V2 AS SELECT b FROM T
                WHERE ts >= NOW() - INTERVAL 50 YEARS AND a < 10 AND ts2 >= NOW() - INTERVAL 2 MONTHS;
                CREATE VIEW V AS
                SELECT b, 1 AS q FROM V1
                UNION ALL
                SELECT b, 2 AS q FROM V2;""";
        var ccs = this.getCCS(sql);
        // Every row is inside both `ts` windows, so `a` and `ts2` decide the result.
        // now - 1 MONTHS is 2024-11-12; now - 2 MONTHS is 2024-10-12.
        ccs.stepWeightOne("""
                        INSERT INTO T VALUES (1, 10, '2024-12-01 00:00:00', '2024-12-01 00:00:00'),
                                             (5, 20, '2024-12-02 00:00:00', '2024-11-01 00:00:00'),
                                             (20, 30, '2024-12-03 00:00:00', '2024-12-03 00:00:00'),
                                             (7, 40, '2024-12-04 00:00:00', '2024-09-01 00:00:00');
                        INSERT INTO now VALUES ('2024-12-12 00:00:00');""",
                """
                          b | q
                        --------
