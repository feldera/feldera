...
public class DeferCarriedColumnsTests extends SqlIoTest {
    ...
    /** A filter above the chain drops row 2; the column it reads is deferred like the others. */
    @Test
    public void droppedRowStaysDropped() {
        Rewritten rewritten = this.compile("""
                ...
                WHERE wide.p5 <> 'drop';""");
        Assert.assertTrue(rewritten.log(),
                rewritten.log().contains("Deferred 5 columns across 4 left joins"));
        // Without the rewrite each of the four joins carries 10 columns
        Assert.assertEquals(List.of(5, 5, 5, 6, 10), this.joinWidths(rewritten.ccs()));
        // Results validated using postgres
        rewritten.ccs().stepWeightOne("""
    ...
    /** A chain headed by a TOP-1 whose first join uses the key.  The collection feeding that
     * join is indexed by the key, so the lookup reuses its index. */
    @Test
    public void lookupReusesTheStartIndex() {
        Rewritten rewritten = this.compile("""
                ...
                CREATE VIEW v AS SELECT latest.id, latest.p1, latest.p2, latest.p3,
                                        l1.v1, l2.v2, l3.v3, l4.v4
                FROM latest
                LEFT JOIN l1 ON latest.id = l1.k
        ...
        Assert.assertEquals(List.of(5, 5, 5, 6, 8), this.joinWidths(rewritten.ccs()));
        // Results validated using postgres:
        // WITH raw(id, ver, a, b, c, d, p1, p2, p3) AS (VALUES (1, 1, 10, 20, 30, 40, 'old', 'old', 'old'),
        //     (1, 2, 10, 20, 30, 40, 'x', 'y', 'z'), (2, 1, 99, 99, 99, 99, 'p', 'q', 'r')),
        //   l1(k, v1) AS (VALUES (1, 'A')), l2(k, v2) AS (VALUES (20, 'B')),
        //   l3(k, v3) AS (VALUES (30, 'C')), l4(k, v4) AS (VALUES (40, 'D')),
        //   latest AS (SELECT DISTINCT ON (id) id, a, b, c, d, p1, p2, p3 FROM raw ORDER BY id, ver DESC)
        // SELECT latest.id, latest.p1, latest.p2, latest.p3, l1.v1, l2.v2, l3.v3, l4.v4 FROM latest
        // LEFT JOIN l1 ON latest.id = l1.k LEFT JOIN l2 ON latest.b = l2.k
        // LEFT JOIN l3 ON latest.c = l3.k LEFT JOIN l4 ON latest.d = l4.k ORDER BY id;
        rewritten.ccs().stepWeightOne("""
