...
public class FilterCorrelateRule
    ...
    @Override public void onMatch(RelOptRuleCall call) {
        ...
        final List<RexNode> inputFilters =
                RelOptUtil.conjunctions(filter.getCondition());

        List<RexNode> leftFilters = new ArrayList<>();
        List<RexNode> rightFilters = new ArrayList<>();
        List<RexNode> aboveFilters = new ArrayList<>();
        ...
        List<RexNode> remainingLeftFilters = new ArrayList<>();
        for (RexNode f: leftFilters) {
            if (CorrelVariableFinder.has(f)) {
                inputFilters.add(f);
            } else {
                remainingLeftFilters.add(f);
            }
        }

        List<RexNode> remainingRightFilters = new ArrayList<>();
        for (RexNode f: rightFilters) {
            if (CorrelVariableFinder.has(f)) {
                inputFilters.add(f);
            } else {
                remainingRightFilters.add(f);
            }
        }

        leftFilters = remainingLeftFilters;
        rightFilters = remainingRightFilters;
        if (leftFilters.isEmpty() && rightFilters.isEmpty()) {
            // no filters got pushed
            return;
        }

        // Do not propagate filters that contain (other) correlation variables
        // leftFilters = Linq.where(leftFilters, f -> !CorrelVariableFinder.has(f));
        // rightFilters = Linq.where(rightFilters, f -> !CorrelVariableFinder.has(f));
