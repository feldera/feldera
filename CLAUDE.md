...
## Context: profiler/CLAUDE.md
<!-- SECTION:profiler/CLAUDE.md START -->
# CLAUDE.md - Profiler

## Overview

The **profiler** is a standalone Node.js/TypeScript visualization application for analyzing Feldera pipeline performance. It provides interactive graph-based visualization of DBSP circuit execution profiles, allowing developers to inspect operator performance, identify bottlenecks, and understand data flow patterns in compiled SQL pipelines.

Currently reads static JSON files from disk; planned integration with the Web Console will enable real-time profiling of running pipelines.

## Purpose

Visualizes two complementary aspects of Feldera pipeline execution:

1. **Profile Data** (`rec.json`): Runtime performance metrics collected from DBSP circuit execution (operator timing, memory usage, throughput, cache statistics)
2. **Dataflow Graph** (`dataflow-rec.json`): Circuit structure generated by SQL compiler showing operators and their connections with source position mappings

The profiler renders this data as an interactive graph using Cytoscape.js with ELK layout, enabling:
- Performance hotspot identification through color-coded nodes
- Drill-down into nested circuit regions
- Worker-level metric comparison
- SQL source code attribution for operators

## Technology Stack

- **TypeScript 5.9**: Strict type-safe implementation with advanced compiler options
- **Cytoscape.js 3.x**: Graph visualization and interaction library
- **cytoscape-elk**: Hierarchical graph layout using Eclipse Layout Kernel
- **Vite 5.x**: Build tool and dev server for fast iteration
- **Live-server**: Development HTTP server
...
Static JSON Files (data/)
  → Profile Parsing (profile.ts)
  → Dataflow Integration (dataflow.ts)
  → Graph Construction (cytograph.ts)
  → Interactive Rendering (Cytoscape + UI)
```
...
**`profile.ts`** (787 lines)
- Parses serialized JSON profile data from Feldera pipeline manager
- Decodes hierarchical circuit structure (simple nodes, clusters, edges)
- Processes worker-level performance measurements with multiple value types:
  - `TimeValue`: Execution duration (seconds/nanoseconds → milliseconds)
  - `NumberValue`: Counts (batches, allocations, records)
  - `PercentValue`: Ratios (cache hit rates, time percentages)
  - `StringValue`: Identifiers (persistent IDs, cache occupancy)
- Merges Z^-1 (delay/trace) nodes split artificially in profile representation
- Computes aggregate measurements for complex (cluster) nodes from children
- Integrates SQL source positions from dataflow graph via persistent node IDs
- Key classes: `CircuitProfile`, `SimpleNode`, `ComplexNode`, `ProfileEdge`, `Measurement`

**`dataflow.ts`** (151 lines)
- Parses dataflow graph JSON from SQL compiler (`--dataflow` flag output)
- Represents MIR (Mid-level IR) node operations with inputs/outputs
- Maps source code positions to circuit operators for debugging
- Provides SQL source fragment extraction with line/column highlighting
- Key classes: `Dataflow`, `MirNode`, `Sources`, `SourcePositionRange`

**`cytograph.ts`** (894 lines)
- Converts `CircuitProfile` to Cytoscape-compatible graph representation
- Manages node expansion/collapse for hierarchical circuits (clusters as parent nodes)
- Computes incremental graph diffs (using `ZSet`) for efficient re-rendering
- Handles edge rewriting when nodes are collapsed into parents
- Renders performance metrics as node colors (percentile-based heatmap: white → red)
- Implements hover tooltips with:
  - Worker-level metric tables with color-coded cells
  - SQL source code fragments
  - Node metadata (operation, persistent ID, parent)
- Highlights reachable edges on hover (forward=red, backward=blue)
- Uses ELK hierarchical layout with configurable strategies
- Key classes: `Cytograph`, `CytographRendering`, `GraphNode`, `GraphEdge`

#### User Interaction

**`selection.ts`** (60 lines)
- Manages which parts of circuit to display (node expansion state)
- Tracks which complex nodes are expanded to show children vs collapsed
...
**`metadataSelection.ts`** (112 lines)
- Controls which performance metrics drive visualization
- Provides UI for selecting:
  - Metric for node coloring (dropdown with all discovered metrics)
  - Worker threads to display (checkboxes, toggle all button)
- Updates rendering when user changes metadata view
- Key classes: `MetadataSelector`, `MetadataSelection`

**`navigator.ts`** (84 lines)
- Minimap widget showing current viewport within full graph
- Displays two nested rectangles:
  - Outer: Full graph bounding box (dark grey)
  - Inner: Current visible viewport (black)
- Scales dynamically as user pans/zooms
- Double-click to fit entire graph in view
- Key class: `ViewNavigator`

#### Utilities

**`util.ts`** (455 lines)
- **`Option<T>`**: Rust-style nullable type with safe unwrapping (avoids null checks)
...
**`planar.ts`** (119 lines)
- Geometric primitives for navigator rendering
...
- Used to compute incremental changes between graph states
- Enables efficient Cytoscape updates (add/remove nodes/edges)
- Maps objects to weights via string encoding

**`globals.ts`** (159 lines)
- Singleton managing application lifecycle and global state
- Fetches JSON files from server with error handling
- Creates and manages tooltip DOM element for hover interactions
- Orchestrates event loop: selector changes → graph recomputation → rendering
- Wires together profile, dataflow, selectors, and rendering
- Key method: `run(profile)` initializes visualization pipeline

**`index.ts`** (4 lines)
- Application entry point
- Loads static files: `data/rec.json` and `data/dataflow-rec.json`
- Temporary implementation; will be replaced by Web Console integration
...
**`index.d.ts`** (2 lines)
- Global type declarations (currently minimal)
...
Vite serves the app at `http://localhost:5173` (typically) with:
- TypeScript compilation on-the-fly
- Module hot replacement for instant updates
...
Place profile and dataflow JSON files in `data/` directory:
- `<name>.json`: Profile data from pipeline manager
- `dataflow-<name>.json`: Dataflow graph from SQL compiler

Update `src/index.ts`:
```typescript
Globals.getInstance().loadFiles("data", "<name>");
```

## Key User Interactions

1. **Metric Selection**: Dropdown to choose which performance metric colors nodes (time%, allocated bytes, cache hit rate, etc.)
2. **Worker Filtering**: Checkboxes to show/hide per-worker measurements
3. **Node Expansion**: Double-click nodes to expand/collapse nested regions
4. **Graph Navigation**: Pan/zoom with mouse; minimap navigator in top-left
5. **Performance Inspection**: Hover over nodes to see:
   - All metrics across visible workers (color-coded table)
   - SQL source code that generated the operator
   - Node metadata (ID, operation type, parent)
6. **Edge Highlighting**: Hovering highlights forward paths (red) and backward paths (blue) from node

## Data Format Requirements

### Profile JSON (`rec.json`)

Generated by Feldera pipeline manager, contains:
- `worker_profiles[]`: Array of per-worker measurements
  - `metadata`: Map of node ID → measurement entries
- `graph`: Circuit structure
  - `nodes`: Hierarchical node tree (Simple/Cluster)
  - `edges`: Connections between nodes

### Dataflow JSON (`dataflow-rec.json`)

Generated by SQL compiler with `--dataflow` flag:
- `sources[]`: SQL source code lines
- `mir`: Map of MIR node ID → operation details
  - `operation`: Operator type (join, filter, etc.)
  - `table`/`view`: Associated schema objects
  - `positions[]`: Source code ranges
  - `persistent_id`: Links to profile nodes

## Performance Characteristics

- **Handles circuits with 100+ operators** efficiently with selective expansion
- **ELK layout** provides high-quality hierarchical graph rendering (slower than force-directed)
- **Incremental graph updates** via ZSet diffing minimize DOM operations
- **Tooltip rendering** can be expensive for nodes with many workers/metrics (40-cell limit per metric)
- **Large profiles** (thousands of nodes) may require pagination or filtering UX

## Known Limitations

- **Static data only**: No live profiling integration yet
- **Single circuit view**: No comparison between profile snapshots
- **No time series**: Shows aggregate metrics, not temporal evolution
- **Worker scaling**: UI becomes cluttered with >50 workers
- **No export**: Cannot save filtered views or generate reports
<!-- SECTION:profiler/CLAUDE.md END -->
