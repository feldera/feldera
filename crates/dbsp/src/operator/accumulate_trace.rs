    ...
    B: Batch<Time = ()>,
{
    ...
    /// Like `shard_accumulate_integrate_trace`, but assigns a "legacy" persistent id to the integral, i.e.,
    /// the same id generated by .shard().accumulate().
    ///
    /// Legacy id: `<pid>.shard.accintegral`
    /// New id: `<pid.shard_accintegral>`
    #[track_caller]
    pub fn shard_accumulate_integrate_trace_legacy(&self) -> Stream<C, Spine<B>> {
        let factories = BatchReaderFactories::new::<B::Key, B::Val, B::R>();

        self.inner()
            .dyn_shard_accumulate_integrate_trace_legacy(&factories)
            .typed()
    }
