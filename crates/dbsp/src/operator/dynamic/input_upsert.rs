    ...
    B: IndexedZSet<Key = T::Key, Val = T::Val>,
{
    async fn eval(
        ...
        let mut keys_builder =
            VecWSetBuilder::with_capacity(&self.keys_factories, n_updates, n_updates);

        let mut builder = B::Builder::with_capacity(&self.batch_factories, n_updates, n_updates);
        ...
        while !updates.is_empty() {
            ...
            if cur_key != Some(key) {
                ...
                // Decide what to do about this key.
                enum Action {
                    /// It's in the in-memory trace, so generate a retraction.
                    GenerateRetraction,

                    /// It's not in the in-memory trace but it might be in the
                    /// on-disk trace.
                    FetchFromStorage,
                }
                ...
                // For an update that is not in the in-memory trace and might be
                // in the on-disk trace, we need to look for it right now
                // anyway, because `patch_func` needs the key's current value.
                let action = if let Some(Action::FetchFromStorage) = action
                    && let UpdateRef::Update(_) = upd.get()
                {
                    trace_cursor
                        .seek_key_exact(key, Some(hash))
                        .then_some(Action::GenerateRetraction)
    ...
    E: DataTrait + ?Sized,
{
    async fn eval(
        ...
        // Current key for which we are processing updates. Borrowed directly
        // from `updates`, which outlives this whole function, so there's no
        // need to clone it into owned storage.
