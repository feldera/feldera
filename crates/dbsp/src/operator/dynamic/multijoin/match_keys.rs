...
/// to all combinations of values under the input cursors.
///
...
///
/// Can hold any resources allocated per step.
...
>
{
    /// Yield one or several output tuples for the current key.
    ///
    /// The callback is invoked for each output tuple.
    ///
    ...
    /// to do so too many times, since Match operator only gets a chance to yield
...
impl<C, I, O, F> MatchInner<C, I, O, F>
where
    C: Circuit,
    I: IndexedZSet,
    O: IndexedZSet,
            ...
            *self.empty_output.borrow_mut() = true;
            ...
            let batch = if size_of::<C::Time>() != 0 {
                ...
                while joint_cursor.next() {
                    ...
                    let mut iter = generator.new_key(&mut joint_cursor.prefix_cursor, &mut joint_cursor.trace_cursors);
                        ...
                        if output_tuples.len() >= chunk_size / 3 {
                            ...
                            if batcher.tuples() >= chunk_size {
                                *self.empty_output.borrow_mut() = false;
                                ...
                                yield (batch, false, /*joint_cursor.position()*/ None /* TODO */);
