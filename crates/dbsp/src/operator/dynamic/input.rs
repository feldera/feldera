...
mod test {
    ...
    /// Randomized stress test: many updates to three keys split across up to three `stage().flush()`
    /// calls per transaction. The accumulated output must match applying updates in order (last
    /// update per key wins within the folded semantics of [`InputUpsert`]).
    #[test]
    fn map_randomized_staged_batches_accumulate_output() {
        let (mut dbsp, (input_handle, output_handle)) = Runtime::init_circuit(1, |circuit| {
