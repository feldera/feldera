...
where
    ...
    Self: Circuit,
{
    // TODO: generalize to take and return multiple streams.
    #[track_caller]
    ...
    where
        F: FnOnce(
            ...
            Stream<NonIterativeCircuit<Self>, TypedBatch<I::Key, I::Val, I::R, I::Inner>>,
        ) -> Result<Stream<NonIterativeCircuit<Self>, O>, SchedulerError>,
        I: Batch,
        O: Batch + Default + std::fmt::Debug,
    {
        ...
        let subcircuit_node_id = self.non_iterative_subcircuit(move |circuit| {
            let accumulated = circuit
                .import_stream(
                    ImportAccumulator::new(&BatchReaderFactories::new::<I::Key, I::Val, I::R>()),
                    &input_stream.try_sharded_version().inner(),
                )
                .typed()
                .apply_owned(|spine: Spine<I>| spine.consolidate());
            accumulated.mark_sharded_if(input_stream);
