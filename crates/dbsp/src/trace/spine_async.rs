...
where
    B: Batch,
{
    pub fn new(notify: &Arc<Notify>, factories: &B::Factories) -> Self {
        Self {
            factories: factories.clone(),
            key_filter: None,
            value_filter: None,
            frontier: B::Time::minimum(),
            slots: std::array::from_fn(|_| Slot::new(notify.clone())),
            request_exit: false,
    ...
    B: Batch,
{
    pub fn new(runtime: Runtime, worker_index: usize, factories: &B::Factories) -> Self {
        ...
        let state = Arc::new(Mutex::new(SharedState::new(&notify, factories)));
        ...
        runtime.tokio_merger_runtime().spawn({
            let state = state.clone();
            let idle = idle.clone();
            let no_backpressure = no_backpressure.clone();
            let local_worker_offset = worker_index - runtime.layout().local_workers().start;
            let buffer_cache =
                runtime.get_buffer_cache(local_worker_offset, ThreadType::Background);
            let slab_allocator =
                runtime.get_fbuf_slab_allocator(local_worker_offset, ThreadType::Background);
            ...
            async move {
                ...
                scope_tokio_merger_locals(worker_index, buffer_cache, slab_allocator, async move {
                    ...
                    let mut mergers = (0..MAX_LEVELS).map(|_| None).collect_vec();
                    let mut worker_state = WorkerState::default();

                    'outer: loop {
                        let mut all_idle = true;
                        for (level, merger) in mergers.iter_mut().enumerate() {
                            let status = Self::run(
                                &mut worker_state,
                                level,
                                max_level0_batch_size_records,
                            ...
                            match status {
                                WorkerStatus::Yield => all_idle = false,
                                WorkerStatus::Idle => (),
                                WorkerStatus::Done => break 'outer,
                            }
                        }

                        if all_idle {
                            // No more merge work currently available -- wait for
                            // the next merges to be started or a global memory
                            ...
                            // Fuel has been spent, but there's still work to do for
                            // the ongoing merge.  Yield control to the tokio
                            // runtime, which will put the task in the end of the
                            // run queue.
                            yield_now().await;
                        }
                    }
    ...
    fn run(
        ...
        if let Some(merger) = opt_merger.as_mut() {
            ...
            if merger.done {
                ...
                let new_level =
                    Spine::<B>::size_to_level(&new_batch, max_level0_batch_size_records, true);
