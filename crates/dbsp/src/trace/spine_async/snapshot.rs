    ...
    B: Batch + Send + Sync,
{
    ...
    /// Snapshots contain multiple batches with independent key ranges, so
    /// there is no single pair of references to return. Callers that need
    /// per-batch bounds should query the individual batches instead.
