...
use crate::{
    ...
    algebra::{
        AddByRef, IndexedZSet, OrdIndexedZSet, OrdIndexedZSetFactories, OrdZSet, OrdZSetFactories,
        ZBatch, ZSet,
    },
    circuit::{CircuitConfig, mkconfig},
    dynamic::{DowncastTrait, DynData, DynUnit, DynWeightedPairs, Erase, LeanVec, pair::DynPair},
    storage::{buffer_cache::CacheStats, file::BatchKeyFilterKind},
    trace::{
        Batch, BatchReader, BatchReaderFactories, Builder, Cursor, FileIndexedWSetFactories,
...
#[test]
fn test_file_wset_roaring_seek_key_exact_skips_absent_reads() {
    let (_temp_dir, mut config) = mkconfig();
    config.dev_tweaks.batch_key_filter = BatchKeyFilterKind::RoaringU32;
