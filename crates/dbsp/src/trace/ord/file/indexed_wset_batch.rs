...
use crate::{
    ...
    storage::{
        ...
        file::{
            ...
            reader::{
                BulkRows, CorruptionError, Cursor as FileCursor, Error as ReaderError, RawItems,
                Reader,
            },
            writer::Writer2,
        },
    ...
    R: WeightTrait + ?Sized,
{
    ...
    fn vals(&self) -> &ValCursor<'s, K, V, R> {
        self.val_cursor.get_or_init(|| {
            match &self.val_hint {
                // Crossing to the value column resolves that column's
                // factories, which sit behind `dyn Any`.  The hint carries a
                // row group over that column with them already resolved, so
                // this takes them from it rather than asking four times over.
                Some(hint) => {
                    let vals = self
                        .key_cursor
                        .next_column_like(hint.row_group())
                        .unwrap_storage();
                    unsafe { vals.first_with_hint(hint) }
                }
                None => {
    ...
    /// Inlined on purpose: a merge steps a key for every key it reads, so this
    /// sits on the hottest path there is here, and its body -- re-pointing the
    /// key cursor and parking the old value cursor as a hint -- is small
    /// enough that the call would be a fair share of it.
    #[inline]
    ...
    R: WeightTrait + ?Sized,
{
    ...
    fn take_values(&mut self, n: u64) {
        let vals = self.vals_mut();
        let to = vals.relative_position() + n;
        unsafe { vals.move_to_row(to) }.unwrap_storage();
    }
