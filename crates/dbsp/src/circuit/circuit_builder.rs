...
pub trait WithClock {
    ...
    ///
    /// Restoring a subcircuit from a checkpoint uses this to put the scope's
    /// clock back where the checkpoint left it; see `ChildNode::restore`.
...
mod tests {
    ...
    use anyhow::anyhow;
    use std::{cell::RefCell, collections::HashMap, ops::Deref, rc::Rc, vec::Vec};
