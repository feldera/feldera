...
mod cpu;
pub use cpu::{CPUProfiler, ParkReason, ParkingFor, RuntimeIdle};
        ...
        for (node_id, meta) in metadata.iter_mut() {
            ...
            if let Some(profile) = self.cpu_profiler.circuit_profile(node_id) {
                let default_meta = metadata![
                    ...
                    CIRCUIT_WAIT_BY_REASON_SECONDS => MetaItem::Map(
                        ParkReason::ALL
                            .iter()
                            .zip(profile.wait_by_reason)
                            .map(|(reason, wait)| {
                                (Cow::Borrowed(reason.name()), MetaItem::Duration(wait))
                            })
                            .collect(),
                    ),
...
mod tests {
    ...
    fn wait_breakdowns(profile: &DbspProfile) -> Vec<(GlobalNodeId, BTreeMap<String, Duration>)> {
        profile
            .worker_profiles
            .iter()
            .flat_map(|worker| worker.attribute_profile(&CIRCUIT_WAIT_BY_REASON_SECONDS))
            .map(|(node_id, breakdown)| {
                let MetaItem::Map(breakdown) = breakdown else {
                    panic!("circuit_wait_by_reason_seconds must be a map: {breakdown:?}");
                };
                let breakdown = breakdown
                    .into_iter()
                    .map(|(reason, parked)| match parked {
                        MetaItem::Duration(parked) => (reason.into_owned(), parked),
                        parked => panic!("a breakdown entry must be a duration: {parked:?}"),
                    })
                    .collect();
                (node_id, breakdown)
            })
