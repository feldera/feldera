//! This module contains a dynamically typed re-implementation of stable and unstable
//! sorting algorithms from the Rust standard library.  DBSP stores data in sorted vectors.
...
// TODO: The unstable sort implementation is currently not used by DBSP.
// - It could do with more testing
...
//   or perhaps my dynamically typed implmentation of unstable sort loses too much performance,
...
fn vec_swap(v: &mut [u8], val_size: usize, index1: usize, index2: usize) {
    ...
    unsafe {
        ptr::swap_nonoverlapping(v.add(index1 * val_size), v.add(index2 * val_size), val_size)
    };
}
...
fn partition_in_blocks(
    ...
    if start_l < end_l {
        ...
        while start_r < end_r {
            ...
            unsafe {
                end_r = end_r.sub(1);
                ptr::swap_nonoverlapping(l, r.sub(val_size * (usize::from(*end_r) + 1)), val_size);
                l = l.add(val_size);
            }
        }
...
#[cfg(test)]
mod test {
    use super::{stable_sort, unstable_sort};
