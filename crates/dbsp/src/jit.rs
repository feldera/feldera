...
//!
//! This module wires a very small LLVM JIT into DBSP.  It can synthesize a
//! function at runtime, treat a raw pointer as the carrier for batches/Z-sets,
//! and invoke the generated function from a DBSP circuit.
...
impl JitFunction {
    fn new(symbol: Arc<str>, func: JitFn, keepalive: SharedJitModule) -> Self {
        Self {
            symbol,
            func,
...
impl LlvmCircuitJit {
    ...
    pub fn compile_add_pipeline(&self, increments: &[i32]) -> Result<JitFunction, JitError> {
        ...
        let typed_ptr = builder
            .build_bitcast(
                raw_arg,
                i32_ty.ptr_type(AddressSpace::default()),
                "batch_ptr",
            )
