    ...
    fn new(
        ...
        Self {
            ...
            pending: blocks
                .iter()
                .map(|r| {
                    Some(PendingRead::new(r.start, {
                        // If this `unwrap` fails, we're trying to read more
                        // than `i32::MAX` bytes in one operation, which we
                        // don't support yet.
                        (r.end - r.start).try_into().unwrap()
                    }))
                })
                .collect(),
            results: vec![None; n],
...
impl PendingRead {
    fn new(offset: u64, len: i32) -> Self {
        Self {
            buffer: ManuallyDrop::new(FBuf::with_capacity(len as usize)),
            offset,
            len,
        }
    }
    ...
    fn complete(self, retval: i32) -> Result<Arc<FBuf>, StorageError> {
        ...
        } else if retval != self.len {
            Err(StorageError::from(IoError::from(ErrorKind::UnexpectedEof)))
        } else {
            unsafe { buffer.set_len(self.len as usize) };
            Ok(Arc::new(buffer))
        }
    }
