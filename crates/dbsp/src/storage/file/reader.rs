...
use crate::{
    ...
    storage::{
        ...
        buffer_cache::{AsyncCacheContext, AtomicCacheStats, CacheStats},
    },
};
use binrw::{
    io::{self},
...
impl TreeNode {
        ...
        A: DataTrait + ?Sized,
    {
        let cache_entry = context
            .read(self.location, |fbuf, location| {
                FileCacheEntry::from_read(fbuf, location, file.compression)
            })
            .await?;
        self.read_tail(cache_entry)
    }
    ...
    T: ColumnSpec,
{
    ...
    pub fn open(
        ...
        Self::new(factories, path.to_path_buf(), cache, file_handle)
    }
    ...
    (&'static K, &'static A, N): ColumnSpec,
{
    ...
    pub fn rows_async<'a>(
        &'a self,
        context: &'a AsyncCacheContext<FileCacheEntry>,
    ...
    A: DataTrait + ?Sized,
{
    ...
    fn root_node(&self) -> Option<TreeNode> {
        self.reader.0.columns[self.column].root.clone()
    }
    ...
    A: DataTrait + ?Sized,
{
        ...
        C: Fn(&K) -> Ordering,
    {
        ...
        loop {
            match node
                .read_async(&row_group.row_group.reader.0.file, &row_group.context)
                .await?
            {
                TreeBlock::Index(index_block) => {
                    let Some(child_idx) =
    ...
    async fn for_row_async<N, T>(
        ...
        Ok(Self::Row(
            Path::for_row_from_ancestor_async(
                &row_group.row_group.reader.0.file,
                row_group.context,
                Vec::new(),
