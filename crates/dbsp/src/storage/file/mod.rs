...
mod test {
        ...
        T: ColumnSpec,
    {
        ...
        assert_eq!(unsafe { cursor.seek_exact(&key) }.unwrap(), true);
        assert_eq!(
            unsafe { cursor.item((tmp_key, tmp_aux)) },
            Some((key.erase_mut(), aux.erase_mut()))
        );
        ...
        T: ColumnSpec,
    {
        ...
        assert_eq!(unsafe { cursor.seek_exact(after) }.unwrap(), false);
        let first2 = unsafe { cursor.item((key2, aux2)) };
        assert_eq!(first1, first2);
        assert_eq!(unsafe { cursor.seek_exact(before) }.unwrap(), false);
        let first2 = unsafe { cursor.item((key2, aux2)) };
        assert_eq!(first1, first2);
    }
