//! Profiling-focused variant of `seek_key_exact` — only the u64 file-backed
//! case, with 10x more lookups to amortise setup cost so the profile shows
//! the lookup hot path clearly.
//!
//! Run with: samply record cargo bench -p dbsp --bench seek_key_exact_profile
//! or: cargo build --bench seek_key_exact_profile --release
//!     perf record -F 4000 -g --call-graph=dwarf ./target/release/deps/seek_key_exact_profile-*
...
#[inline(never)]
fn run_lookups(zset: &OrdZSet<u64>, lookups: &[u64]) -> (u64, u64) {
    let mut cursor = zset.inner().cursor();
...
fn main() {
    ...
    let handle = Runtime::run(config, move |_parker| {
        ...
        let (hits, elapsed_ns) = run_lookups(&zset, &lookups);
    ...
    println!(
        "file-backed: {NUM_LOOKUPS} lookups against {keys} keys in {:.2} ms = {:.1} ns/lookup, {} hits",
