...
struct Args {
    ...
    /// Minimum size of the key blocks in the batches the lazy map's
    /// accumulator writes for itself, in KiB.  A power of two.  Unset takes
    /// the runtime's default of 32; 8 is the file writer's default.
    #[arg(long)]
    ...
    Tup2<i64, Update<V, V>>: dbsp::DBData,
{
    ...
    config.dev_tweaks.lazy_input_map_key_block_bytes = args.key_block_kib.map(|kib| kib << 10);
