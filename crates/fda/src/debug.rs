...
async fn unbundle_support_bundle(
    ...
    // Create a helper function to extract timestamp from filename
    let extract_timestamp_prefix =
        |filename: &str| -> Option<String> { filename.split('_').next().map(|s| s.to_string()) };

    // Find all pipeline config files and their corresponding platform versions
    let mut pipeline_configs = Vec::new();
    let mut platform_versions = std::collections::HashMap::new();
    ...
    for i in 0..archive.len() {
        ...
        } else if filename.ends_with("_system_config.json") {
            if let Some(timestamp) = extract_timestamp_prefix(&filename) {
                let mut file = archive.by_index(i)?;
                let mut contents = String::new();
                file.read_to_string(&mut contents)?;

                if let Ok(system_config) = serde_json::from_str::<Value>(&contents) {
                    if let Some(version) = system_config["version"].as_str() {
                        let version_tag = format!("v{}", version);
                        platform_versions.insert(timestamp.clone(), version_tag.clone());
                        debug!(
                            "Found platform version {} for timestamp {}",
    ...
    for config_file in pipeline_configs.iter() {
            ...
            &instance_config
        {
            if *supports_runtime_version {
                corresponding_platform_version.clone()
            } else {
                if let Some(bundle_version_str) = &corresponding_platform_version {
                    if bundle_version_str.trim_start_matches('v') != instance_version {
            ...
            corresponding_platform_version.clone() // Use it in dry-run or if we couldn't check
        };
        ...
        if dry_run {
            ...
            } else if corresponding_platform_version.is_some() {
