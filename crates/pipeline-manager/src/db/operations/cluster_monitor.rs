...
fn row_to_extended_cluster_monitor_event(
    ...
    Ok(ExtendedClusterMonitorEvent {
        ...
        runner_status: row.get::<_, String>(5).try_into()?,
        runner_self_info: row.get(6),
        runner_resources_info: row.get(7),
        compiler_status: row.get::<_, String>(8).try_into()?,
        compiler_self_info: row.get(9),
        compiler_resources_info: row.get(10),
    })
}

/// Only the status cluster monitor event columns.
const RETRIEVE_SHORT_CLUSTER_EVENT_COLUMNS: &str =
    "e.id, e.recorded_at, e.api_status, e.runner_status, e.compiler_status";
...
fn row_to_short_cluster_monitor_event(row: &Row) -> Result<ClusterMonitorEvent, DBError> {
    ...
    Ok(ClusterMonitorEvent {
        ...
        recorded_at: row.get(1),
        api_status: row.get::<_, String>(2).try_into()?,
        runner_status: row.get::<_, String>(3).try_into()?,
        compiler_status: row.get::<_, String>(4).try_into()?,
    })
}
...
pub(crate) async fn new_cluster_monitor_event(
    ...
    txn.execute(
        ...
        &[
            ...
            &event_descr.runner_status.to_string(),   // $5: runner_status
            &event_descr.runner_self_info,            // $6: runner_self_info
            &event_descr.runner_resources_info,       // $7: runner_resources_info
            &event_descr.compiler_status.to_string(), // $8: compiler_status
            &event_descr.compiler_self_info,          // $9: compiler_self_info
            &event_descr.compiler_resources_info,     // $10: compiler_resources_info
        ],
    )
    .await
