...
use crate::db::operations::pipeline::{get_pipeline_by_id, get_pipeline_for_monitoring};
...
use crate::db::types::utils::validate_storage_status_details;
...
const RETRIEVE_EXTENDED_PIPELINE_EVENT_COLUMNS: &str = "e.id, e.recorded_at,
     ...
     e.runtime_status, e.runtime_status_details, e.runtime_desired_status, e.error,
     e.program_status, e.storage_status, e.storage_status_details";
...
fn row_to_extended_pipeline_monitor_event(
    ...
    let error = match row.get::<_, Option<String>>(8) {
        None => None,
        Some(s) => Some(deserialize_error_response(&s)?),
    };
    let storage_status_details = match row.get::<_, Option<String>>(11) {
        None => None,
        Some(s) => Some(deserialize_json_value(&s)?),
    };
    ...
    Ok(ExtendedPipelineMonitorEvent {
        ...
        storage_status: row.get::<_, String>(10).try_into()?,
...
const RETRIEVE_SHORT_PIPELINE_EVENT_COLUMNS: &str = "e.id, e.recorded_at,
    ...
    e.runtime_status, e.runtime_desired_status, e.error IS NOT NULL,
...
fn row_to_short_pipeline_monitor_event(row: &Row) -> Result<PipelineMonitorEvent, DBError> {
    ...
    Ok(PipelineMonitorEvent {
        ...
        program_status: row.get::<_, String>(7).try_into()?,
        storage_status: row.get::<_, String>(8).try_into()?,
    })
}
        ...
        .prepare_cached(
            "INSERT INTO pipeline_monitor_event
                (
                    ...
                    runtime_status, runtime_status_details, runtime_desired_status, error,
