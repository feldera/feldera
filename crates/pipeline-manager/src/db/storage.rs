...
pub(crate) trait Storage {
    ...
    /// Transitions program status from `CompilingSql` to `Success` for a Gen-2
    ...
    /// `CompilingRust`. The predicate must stay the union of the four worker
    /// queries; the full invariant is documented at
