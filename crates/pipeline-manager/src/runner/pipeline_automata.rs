...
use crate::db::types::pipeline::{
    ExtendedPipelineDescr, ExtendedPipelineDescrMonitoring, PipelineId, PipelineStatus,
};
...
use feldera_types::runtime_status::{
    decide_pipeline_action_needed, ExtendedRuntimeStatus, RuntimeStatus,
};
...
impl<T: PipelineExecutor> PipelineAutomaton<T> {
    ...
    /// Timeout for an HTTP request to the pipeline action endpoints (`/activate`, `/start`,
    /// `/pause`, `/suspend`).
    const PIPELINE_ACTION_HTTP_REQUEST_TIMEOUT: Duration = Duration::from_secs(5);
    ...
    async fn do_run(&mut self) -> Result<Duration, DBError> {
        ...
        let transition: State = match (
            ...
            pipeline.deployment_status.as_resources_status(),
            pipeline
        ...
        let new_status = match &transition {
                    ...
                    .await
                {
                    ...
                    Err(e) => match e {
                        DBError::OutdatedPipelineVersion {
                            ...
                            if pipeline.deployment_status.as_resources_status()
                                != ResourcesStatus::Stopped
                            {
                                error!("Outdated pipeline version occurred when transitioning from {} to Provisioning (not Stopped)", pipeline.deployment_status);
                            }
            ...
            State::TransitionToProvisioned {
                ...
                self.db
                    .lock()
                    .await
                    .transit_deployment_status_to_provisioned(
                        self.tenant_id,
                        pipeline.id,
                        version_guard,
                        deployment_location,
                ...
                match extended_runtime_status.status {
                    ...
                    RuntimeStatus::Suspended => {
                        error!("Automaton should not have attempted to transition to Suspended runtime status");
        ...
        let poll_timeout = match new_status.as_resources_status() {
        ...
        match self.pipeline_handle.is_provisioned().await {
            ...
            Ok(None) => {
                ...
                if Utc::now().timestamp_millis()
                    - pipeline.deployment_status_since.timestamp_millis()
        ...
        match result {
            Ok((status_code, body)) => {
                ...
                if status_code == StatusCode::OK {
                    if let Some(response_str) = body.as_str() {
                        ...
                        match response_str {
                            ...
                            _ => Ok(ExtendedRuntimeStatus {
                                ...
                                details: format!("Pipeline status response (200 OK) is an unexpected JSON string: '{response_str}'"),
                            }),
                        }
                        ...
                        match serde_json::from_value::<ExtendedRuntimeStatus>(body) {
                            ...
                            Err(e) => Ok(ExtendedRuntimeStatus {
                                status: RuntimeStatus::Unavailable,
                                details: format!("Pipeline status response (200 OK) cannot be deserialized due to: {e}"),
                            }),
                        }
                    } else {
                        ...
                        Ok(ExtendedRuntimeStatus {
                            status: RuntimeStatus::Unavailable,
                            details: format!("Pipeline status response (200 OK) is not a string or an object:\n{body:#}"),
                        })
                    }
                    ...
                    match serde_json::from_value::<ErrorResponse>(body.clone()) {
                        Ok(error_response) => {
                            match error_response.error_code.as_ref() {
                                ...
                                _ => {
                                    ...
                                    Ok(ExtendedRuntimeStatus {
                                        status: RuntimeStatus::Unavailable,
                                        details: format!("Pipeline status response (503 Service Unavailable) is an error:\n{error_response:?}"),
                                    })
                                },
                            }
                        }
                        Err(e) => Ok(ExtendedRuntimeStatus {
                            status: RuntimeStatus::Unavailable,
                            details: format!("Pipeline status response (503 Service Unavailable) cannot be deserialized due to: {e}. Response was:\n{body:#}"),
                        })
                    }
                } else {
            ...
            Err(e) => {
                    ...
                    .is_some_and(|t| t.elapsed() <= Self::PROVISIONED_GRACE_PERIOD)
                {
                    Ok(ExtendedRuntimeStatus {
                        status: RuntimeStatus::Initializing,
                        details: format!("Still in the grace period for initializing. Pipeline status endpoint cannot yet be reached due to: {e}")
                    })
                } else {
                    warn!(
    ...
    async fn observe_provisioned_pipeline_and_enforce_runtime_status(
        ...
        // Retrieve status
        let first_retrieved_status = self.parse_status_result(
        ...
        match first_retrieved_status {
            Ok(extended_runtime_status) => {
                // The pipeline did not experience a runtime error, and as such provided a status.
                // We determine now whether a runtime action needs to be taken based on the current
                // runtime status and the desired status.
                let Some(runtime_desired_status) = pipeline
                    .deployment_desired_status
                    .as_runtime_desired_status()
                else {
                    return Self::provisioned_transition_using_runtime_status(
                        pipeline,
                        deployment_location,
                        extended_runtime_status,
                    );
                };
                let Some((method, action)) = decide_pipeline_action_needed(
                    extended_runtime_status.status,
                ...
                // Perform the action, though if it failed, it will not cause the pipeline to go
                // down unless the subsequent status response also fails
                match self
                    .http_request_pipeline_json(
                        method,
                        deployment_location,
                        &action,
                        Self::PIPELINE_ACTION_HTTP_REQUEST_TIMEOUT,
                    )
                    .await
                {
                    Ok((status_code, body)) => {
                        if status_code != StatusCode::OK {
                            error!("Error returned by pipeline {} for action /{action}: status code {status_code} with body:\n{body:#}", pipeline.id);
                        }
                    }
                    Err(e) => {
                        error!(
                            "Error returned by pipeline {} for action /{action}: {e}",
                            pipeline.id
                        );
                    }
                }

                // After the action, we again retrieve the status and use that as final determined
                // status
                let second_retrieved_status = self.parse_status_result(
                    self.http_request_pipeline_json(
                        Method::GET,
                        deployment_location,
                        "status",
                ...
                match second_retrieved_status {
                    Ok(extended_runtime_status) => {
                        Self::provisioned_transition_using_runtime_status(
                            pipeline,
                            deployment_location,
                            extended_runtime_status,
                        )
    ...
    /// Based on the provided runtime status while `Provisioned`, decides which transition occurs.
    fn provisioned_transition_using_runtime_status(
        pipeline: &ExtendedPipelineDescrMonitoring,
        deployment_location: &str,
        extended_runtime_status: ExtendedRuntimeStatus,
    ) -> State {
        if let Some(prev_runtime_status) = pipeline.deployment_status.as_runtime_status() {
            if prev_runtime_status == extended_runtime_status.status {
                return State::Unchanged;
            }
        }
        if extended_runtime_status.status == RuntimeStatus::Suspended {
            return State::TransitionToStopping {
                error: None,
