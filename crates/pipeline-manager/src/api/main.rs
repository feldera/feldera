...
- If it is behind a (reverse) proxy, the proxy can return error codes by itself,
...
_Client behavior:_ clients should immediately return with an error when they get back a 4xx status
code, as it means the request will very likely not succeed even if it is sent again.
