...
#[cfg(test)]
mod tests {
    use super::{parse_tenant_id, parse_user_id};
    use crate::db::error::DBError;
    use crate::db::types::tenant::TenantId;
    ...
    fn parse_tenant_id_accepts_uuid() {
        let id = Uuid::nil();
        assert_eq!(parse_tenant_id(&id.to_string()).unwrap(), TenantId(id));
    }

    #[test]
    fn parse_tenant_id_unknown_for_non_uuid() {
        assert!(matches!(
            parse_tenant_id("not-a-uuid"),
            Err(ManagerError::DBError {
                db_error: DBError::UnknownTenantName { name }
            }) if name == "not-a-uuid"
        ));
    }
    ...
    #[test]
    fn parse_user_id_unknown_for_non_uuid() {
        assert!(matches!(
            parse_user_id("not-a-uuid"),
            Err(ManagerError::DBError {
                db_error: DBError::UnknownUser { user_id }
            }) if user_id == "not-a-uuid"
        ));
    }
}
