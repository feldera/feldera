...
impl ClusterMonitorEventSelectedInfo {
    fn new_all(event: ExtendedClusterMonitorEvent) -> Self {
        ClusterMonitorEventSelectedInfo {
            ...
            all_healthy: (event.api_status, event.runner_status, event.compiler_status)
                == (
...
/// Default for the `selector` query parameter when GET a cluster monitor event.
fn default_cluster_monitor_event_field_selector() -> ClusterMonitorEventFieldSelector {
    ClusterMonitorEventFieldSelector::All
}
...
pub struct GetClusterEventParameters {
    ...
    /// in status.
    #[serde(default = "default_cluster_monitor_event_field_selector")]
...
#[utoipa::path(
    context_path = "/v0",
    security(("JSON web token (JWT) or API key" = [])),
    params(GetClusterEventParameters),
    params(
...
pub(crate) async fn get_cluster_health(
    ...
    if health_status.api.healthy && health_status.runner.healthy && health_status.compiler.healthy {
