...
fn create_auth_json_error(message: &str) -> actix_web::Error {
...
async fn bearer_auth(
    ...
    let token = match configuration.provider {
        ...
        AuthProvider::Okta(_) => decode_okta_token(token, &req, configuration).await,
    };
    match token {
        Ok(claim) => {
            // Validate groups authorization (Okta only)
            ...
            {
                return Err((
                    create_auth_json_error("User does not belong to required groups for access."),
                    req,
                ));
            }
            ...
            let tenant_name = match claim.tenant_name(&state.config) {
                ...
                Err(AuthError::NoTenantFound) => {
                    return Err((
                        create_auth_json_error("You are not authorized to access any Feldera tenant. Contact your administrator if you need access to Feldera."),
                        req,
                    ));
                }
                Err(e) => {
                    ...
                    return Err((
                        create_auth_json_error(&format!("Tenant resolution failed: {}", e)),
                        req,
                    ));
                }
            };
        ...
        Err(error) => {
            ...
            Err((create_auth_json_error(&descr), req))
        }
    }
}
...
async fn api_key_auth(
    ...
    match validate {
        ...
        Err(_) => Err((create_auth_json_error("Unauthorized API key"), req)),
    }
}
...
enum Claim {
    ...
    Okta(TokenData<OidcClaim>),
    GoogleIdentity(TokenData<OidcClaim>),
}
...
impl Claim {
    fn tenant_name(&self, config: &crate::config::ApiServerConfig) -> Result<String, AuthError> {
        ...
        let (tenant_claim, issuer, sub) = match self {
            ...
            Claim::Okta(t) => (&t.claims.tenant, &t.claims.iss, &t.claims.sub),
            Claim::GoogleIdentity(t) => (&t.claims.tenant, &t.claims.iss, &t.claims.sub),
        };
    ...
    fn provider(&self) -> String {
        match self {
            ...
            Claim::Okta(t) => t.claims.iss.clone(),
            Claim::GoogleIdentity(t) => t.claims.iss.clone(),
        }
    }
}
...
///
/// Extracts the subdomain from issuer URLs for tenant identification.
/// Examples:
/// - "https://acme-corp.okta.com/oauth2/default" → Some("acme-corp")
/// - "https://company.auth.us-west-2.amazoncognito.com/oauth2" → Some("company")
/// - "https://accounts.google.com" → Some("accounts")
/// Validates that the user belongs to at least one required group (Okta only).
fn validate_groups_authorization(claim: &Claim, config: &ApiServerConfig) -> Result<(), AuthError> {
    // Only validate groups for Okta provider
    let groups = match claim {
        Claim::Okta(token) => &token.claims.groups,
        _ => return Ok(()), // No group validation for other providers
    };
...
pub(crate) fn okta_auth_config(api_config: &crate::config::ApiServerConfig) -> AuthConfiguration {
    let mut validation = Validation::new(Algorithm::RS256);
    let client_id =
        env::var("AUTH_CLIENT_ID").expect("Missing environment variable AUTH_CLIENT_ID");
    let iss = env::var("AUTH_ISSUER").expect("Missing environment variable AUTH_ISSUER");
    let jwk_uri = format!("{}/v1/keys", iss);
    validation.set_issuer(&[&iss]);
    // Expect feldera-api as the audience claim from Okta custom authorization server
    validation.set_audience(&["feldera-api"]);
    ...
    AuthConfiguration {
        provider: AuthProvider::Okta(ProviderOkta {
            jwk_uri,
...
struct OidcClaim {
    /// The audience for the token (client_id)
    aud: Option<String>,
    ...
    T: for<'de> serde::Deserialize<'de> + serde::Serialize,
{
    ...
    match header {
        ...
        Err(e) => {
            debug!("Failed to decode JWT header: {:?}", e.kind());
            Err(e.into())
        }
    }
}
...
/// Okta-specific token decoder
async fn decode_okta_token(
    token: &str,
    req: &ServiceRequest,
    configuration: &AuthConfiguration,
) -> Result<Claim, AuthError> {
    let result = decode_oidc_token::<OidcClaim>(
        ...
        configuration,
        Claim::Okta,
        None, // Okta doesn't require token_use validation
    )
    .await;
    ...
    match &result {
        Ok(..) => {}
        Err(e) => {
            debug!("Failed to decode Okta token: {}", e);
        }
    }
...
async fn decode_google_identity_token(
    ...
    decode_oidc_token::<OidcClaim>(
        token,
        req,
        configuration,
        Claim::GoogleIdentity,
        None, // Google doesn't require token_use validation
    )
    .await
}
...
async fn fetch_jwk_keys(
    ...
    match &provider {
        ...
        AuthProvider::GoogleIdentity(provider) => fetch_jwk_oidc_keys(&provider.jwk_uri).await,
        AuthProvider::Okta(provider) => fetch_jwk_oidc_keys(&provider.jwk_uri).await,
    }
}
...
async fn fetch_jwk_oidc_keys(url: &String) -> Result<HashMap<String, DecodingKey>, AuthError> {
    ...
    match &res {
        Ok(response) => {}
        Err(e) => debug!("JWK fetch request failed: {:?}", e),
    }
