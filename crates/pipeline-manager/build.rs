...
const EXCLUDE_LIST: [&str; 4] = [
    "../../packages/web-console/node_modules",
    "../../packages/web-console/build",
    "../../packages/web-console/.svelte-kit",
    "../../packages/web-console/pipeline-manager-",
];
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Ok(webui_out_folder) = env::var("WEBCONSOLE_BUILD_DIR") {
        ...
        ChangeDetection::exclude(|path: &Path| {
            ...
            // of it
            || path.to_str().unwrap() == "../../packages/web-console/"
        })
        ...
        let asset_path: PathBuf = Path::new("../../packages/web-console/").join(rel_build_dir);
        let mut resource_dir = NpmBuild::new("../../packages/web-console")
            .executable("bun")
            .run("clean-install")
            .expect("Could not run `bun ci`. Follow set-up instructions in packages/web-console/README.md")
            .run("build")
            .expect("Could not run `bun run build`. Run it manually in packages/web-console/ to debug.")
