...
impl TestClient {
    ...
    const PIPELINE_RESUME_TIMEOUT: Duration = Duration::from_secs(30);
    ...
    pub async fn start_running_pipeline(&self, pipeline_name: &str, wait: bool) {
        assert_eq!(
            self.post_no_body(&format!(
                "/v0/pipelines/{pipeline_name}/start?initial=running"
            ))
            .await
            .status(),
            StatusCode::ACCEPTED
    ...
    pub async fn start_paused_pipeline(&self, pipeline_name: &str, wait: bool) {
        assert_eq!(
            self.post_no_body(&format!(
                "/v0/pipelines/{pipeline_name}/start?initial=paused"
            ))
            .await
            .status(),
            StatusCode::ACCEPTED
    ...
    pub async fn start_standby_pipeline(&self, pipeline_name: &str, wait: bool) {
        assert_eq!(
            self.post_no_body(&format!(
                "/v0/pipelines/{pipeline_name}/start?initial=standby"
            ))
            .await
            .status(),
            StatusCode::ACCEPTED
    ...
    pub async fn resume_pipeline(&self, pipeline_name: &str, wait: bool) {
        assert_eq!(
            self.post_no_body(&format!("/v0/pipelines/{pipeline_name}/resume"))
                .await
                .status(),
            StatusCode::ACCEPTED
        );
    ...
    pub async fn stop_pipeline(&self, pipeline_name: &str, wait: bool) {
        let response = self
            .post_no_body(&format!("/v0/pipelines/{pipeline_name}/stop?force=false"))
            .await;
        let (status_code, body) = Self::parse_json_response(response).await;
        assert_eq!(
            status_code,
            StatusCode::ACCEPTED,
            "Stop without force failed: {body:#}"
        );
        if wait {
            self.wait_for_pipeline_status(
                pipeline_name,
    ...
    pub async fn prepare_pipeline(&self, pipeline_name: &str, sql: &str) {
        ...
        self.start_paused_pipeline(pipeline_name, true).await;
    }
