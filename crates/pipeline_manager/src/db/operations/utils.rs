...
/// Converts an UNIX timestamp to a `DateTime<Utc>` object.
///
/// The UNIX timestamps themselves are generated by the database itself,
/// as such there is no counterpart to this function to convert back.
pub(crate) fn convert_unix_timestamp_to_datetime(
    column: &str,
    epoch_timestamp: i64,
) -> Result<DateTime<Utc>, DBError> {
    DateTime::from_timestamp(epoch_timestamp, 0).ok_or_else(|| {
        DBError::invalid_data(format!(
            "Invalid epoch timestamp {epoch_timestamp} in column {column}"
        ))
    })
}
