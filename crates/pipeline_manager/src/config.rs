...
fn default_jit_pipeline_runner_path() -> String {
    "pipeline".to_string()
}
...
impl LocalRunnerConfig {
    ...
    /// Location to write the SQL code.
    pub(crate) fn sql_file_path(&self, pipeline_id: PipelineId) -> PathBuf {
        self.pipeline_dir(pipeline_id).join("program.sql")
    }

    /// Location of the schema file generated by the SQL compiler.
    pub(crate) fn schema_file_path(&self, pipeline_id: PipelineId) -> PathBuf {
        self.pipeline_dir(pipeline_id).join("schema.json")
    }

    /// Location of the JIT IR file generated by the SQL compiler.
    pub(crate) fn jit_ir_file_path(&self, pipeline_id: PipelineId) -> PathBuf {
        self.pipeline_dir(pipeline_id).join("ir.json")
    }
