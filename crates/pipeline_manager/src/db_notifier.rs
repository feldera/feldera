...
// #[cfg(not(test))]
use log::{debug, error, trace, warn}; // Use log crate when building application

// Useful for testing
// #[cfg(test)]
// use std::{println as warn, println as debug, println as trace, println as error};
...
/// Listen for changes to a table (currently set up for the program, pipeline and
/// pipeline_runtime_state tables).
///
/// Partly inspired by the Kubernetes Informers machinery.
///
/// Calling listen() subscribes to a stream of `DbNotification` messages, received on the rx-half
/// of the tx `UnboundedSender`. listen() will retry in case of connection failures. After
/// establishing a connection, it will first issue a notification for every entry in the subscribed
/// tables. These will show up as `Operation::Add` events.
///
/// TODO: Unlike Kubernetes informers, we do not supply the entire object per event. Ideally, we'd
...
pub(crate) async fn listen(
    ...
    loop {
        ...
        // Not clear why the calls to LISTEN block forever when we call it from the same thread.
        // But we need to call LISTEN before we can poll for async notifications
            ...
            match stream.next().await {
                Some(Ok(msg)) => {
                    ...
                    match msg {
                        ...
                        _ => warn!("Receieved AsyncMessage that isn't a notice or notification"),
                    }
                }
                // Both Err and None are terminal messages. We should exit then.
                Some(Err(e)) => {
                    println!("Reconciler received an error: {:?}", e);
                    break;
                }
                None => {
                    println!("Reconciler received 'None'");
                    break;
                }
            }
        }
