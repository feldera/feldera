...
use crate::{config::InputEndpointConfig, suspend::SuspendError, transaction::TransactionId};
...
pub struct TransactionInitiators {
    ...
    pub initiated_by_api: Option<TransactionPhase>,
    /// Transaction phases initiated by connectors, indexed by endpoint name.
    pub initiated_by_connectors: BTreeMap<String, ConnectorTransactionPhase>,
}
...
/// Output endpoint configuration (minimal).
#[derive(Clone, Debug, Serialize, Deserialize, ToSchema)]
...
/// `#[schema(as = feldera_types::adapter_stats::ControllerStatus)]` to
