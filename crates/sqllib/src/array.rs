...
#[doc(hidden)]
pub fn array_map_safe__<T, S, F>(vec: Array<T>, f: F) -> SqlResult<Array<S>>
where
    F: Fn(&T) -> SqlResult<S>,
{
    (*vec)
...
#[doc(hidden)]
pub fn array_map_safeN_<T, S, F>(vec: Option<Array<T>>, f: F) -> SqlResult<Array<S>>
where
    F: Fn(&T) -> SqlResult<S>,
{
    match vec {
        // Treat like an error; the result will be unwrapped to None
        None => Err(SqlRuntimeError::from_strng("empty result")),
