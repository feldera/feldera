    ...
    S: BuildHasher + Clone,
{
    ...
    pub fn with_hasher(total_capacity_bytes: usize, num_shards: usize, hash_builder: S) -> Self {
        ...
        #[cfg(test)]
        let quick_cache_hash_builder = hash_builder.clone();
        #[cfg(not(test))]
        let quick_cache_hash_builder = hash_builder;

        Self {
            cache: QuickCache::with_options(
                options,
                CacheEntryWeighter,
                quick_cache_hash_builder,
                Default::default(),
            ),
            #[cfg(test)]
            hash_builder,
    ...
    pub(crate) fn validate_invariants(&self) {
        ...
        for (idx, used) in shard_usage.into_iter().enumerate() {
            ...
            assert_eq!(
                reported_capacity,
                self.cache.shard_capacity() as usize,
                "per-shard capacity mismatch"
            );
            assert!(
