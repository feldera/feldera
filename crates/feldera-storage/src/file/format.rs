...
//! * [`DataBlockHeader`].
//!
//! * [`Item<K, A>`] data items serialized with [`rkyv`].  The number of these
//!   is specified in [`DataBlockHeader::n_values`].
//!
//! * The "value map", which holds a byte offset from the start of the block to
//!   the start of each [`Item`].  The offset to the value map is specified in
//!   [`DataBlockHeader::value_map_ofs`] and the format in
...
pub struct DataBlockHeader {
    ...
    /// beginning of the block to the [`Item<K, A>`] that represents the `i`
