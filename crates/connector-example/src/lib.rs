...
//!
//! 4. Declare a `pub static` annotated with
//!    `#[linkme::distributed_slice(feldera_adapterlib_meta::CONNECTOR_METADATA_REGISTRY)]`.
//!    This registers the connector's metadata (name, direction, schema, …)
//!    without depending on `feldera-adapterlib` — only
//!    `feldera-adapterlib-meta` is required.  The runtime builder fn
//!    (`build_*`) is kept in a separate `#[cfg(feature = "impl")]` module so
//!    the describer can compile the crate in seconds with `default-features =
//!    false`, touching only the metadata side.
//!
...
// ── Descriptor callbacks (cheap, no heavy deps) ───────────────────────────────
...
//
// This static is unconditional: it compiles under both `default-features =
// false` (describer / metadata-only mode) and the default `impl` build.
// The describer links this crate with `extern crate connector_example as _;`,
// which keeps the rlib alive so the linkme section fires and `HELLO_LINES_META`
// contributes to `CONNECTOR_METADATA_REGISTRY`.

/// Connector metadata for `hello_lines`.
#[linkme::distributed_slice(feldera_adapterlib_meta::CONNECTOR_METADATA_REGISTRY)]
pub static HELLO_LINES_META: ConnectorDescriptor = ConnectorDescriptor {
    name: "hello_lines",
...
// ── Implementation (compiled only when feature = "impl") ─────────────────────
//
// Gate the entire runtime implementation behind `#[cfg(feature = "impl")]`.
// Anything in this module depends on `feldera-adapterlib` and other heavy deps
// that are optional in `Cargo.toml`.  The describer builds this crate with
// `default-features = false`, so none of this code is compiled during
// describer builds — keeping the describer fast.

#[cfg(feature = "impl")]
mod hello_lines_impl {
    use super::HelloLinesConfig;
    ...
    pub(super) struct HelloLinesEndpoint {
        pub config: HelloLinesConfig,
    }
    ...
    pub(super) struct HelloLinesReader {
        pub sender: UnboundedSender<InputReaderCommand>,
    ...
    pub(super) fn encode_record(col_name: &str, line: &str) -> Vec<u8> {
    ...
    /// Build a `hello_lines` input endpoint from its JSON config.
    pub fn build_hello_lines(
        config: &JsonValue,
        _endpoint_name: &str,
...
/// Build a `hello_lines` input endpoint from its JSON config.
///
/// Available only when the `impl` feature is enabled (the default).  The
/// describer workspace builds this crate with `default-features = false` so
/// that only the descriptor metadata is compiled — `build_hello_lines` is not
/// available in that mode.
#[cfg(feature = "impl")]
pub use hello_lines_impl::build_hello_lines;
...
mod tests {
    ...
    // Config tests — unconditional; `HelloLinesConfig` only needs `serde`.
    ...
    // Metadata registry test — unconditional; HELLO_LINES_META uses only
    // feldera-adapterlib-meta, which is always compiled.

    #[test]
    fn connector_is_in_metadata_registry() {
        use feldera_adapterlib_meta::{descriptor_by_name, Direction, ConnectorKind};

        let d = descriptor_by_name("hello_lines")
            .expect("hello_lines not found in CONNECTOR_METADATA_REGISTRY");
        assert_eq!(d.direction, Direction::Input);
        assert_eq!(d.kind, ConnectorKind::Regular);
        assert_eq!(d.fault_tolerance, Some(FtModel::ExactlyOnce));
        assert!(d.default_format.is_some());
    }

    // Impl-only tests — only compiled when feature = "impl" is active.

    #[cfg(feature = "impl")]
    mod impl_tests {
        use super::super::hello_lines_impl::{StepMeta, encode_record};
        use feldera_adapterlib::transport::parse_resume_info;
