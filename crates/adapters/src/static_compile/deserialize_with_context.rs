...
//!
//! To support this, we need to implement configurable deserializers that can
//! change their behavior based on a user-defined profile.  Unfortunately,
//! serde's `Deserialize` trait doesn't allow this.  The only support `serde`
//! provides for passing state to deserializers is the `DeserializeSeed` trait,
//! but there is no support for automatically threading it through nested types
//! or to auto-derive implementations using it.
//!
//! ## Design
//!
...
//! by passing the context to each elements.
//!
...
///
/// Unlike [`deserialize_struct`], this macro:
...
macro_rules! deserialize_table_record {
    ($table:ident[$sql_table:tt, $num_cols:expr]{$(($field_name:ident, $column_name:tt, $case_sensitive:tt, $type:ty, $init:expr)),* }) => {
        ...
        #[allow(unused_mut)]
        #[allow(dead_code)]
        impl<'de> $crate::DeserializeWithContext<'de, $crate::SqlDeserializerConfig> for $table {
            fn deserialize_with_context<D>(deserializer: D, context: &'de $crate::SqlDeserializerConfig) -> Result<Self, D::Error>
            where
                D: serde::Deserializer<'de>,
            {
                struct RecordVisitor<'de> {
                    context: &'de $crate::SqlDeserializerConfig,
                }

                impl<'de> RecordVisitor<'de> {
                    fn new(context: &'de $crate::SqlDeserializerConfig) -> Self {
                        Self {
                            context
                        }
                    }
                ...
                impl<'de> serde::de::Visitor<'de> for RecordVisitor<'de> {
                        ...
                        while let Some(column_name) = map.next_key::<std::borrow::Cow<'de, str>>()? {
                            ...
                            $(
                                if $column_name == (if $case_sensitive { column_name.as_ref() } else { &uppercase_column_name } ) {
                                    ...
                                    $field_name = Some(map.next_value_seed(<$crate::DeserializationContext<$crate::SqlDeserializerConfig, $type>>::new(self.context))
                    ...
                    where A: serde::de::SeqAccess<'de> {
                        ...
                        let result = $table {
                            $($field_name: {
                                _cols += 1;
                                seq.next_element_seed(<$crate::DeserializationContext<$crate::SqlDeserializerConfig, $type>>::new(self.context))
                                    .map_err(|e| {
