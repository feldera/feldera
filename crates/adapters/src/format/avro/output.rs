...
use crate::format::workerpool::{ConsumerMessage, WorkerFn, Workerpool};
...
impl WorkerFn for AvroParallelEncoder {
    fn process(&mut self, cursor: SplitCursorBuilder) -> AnyResult<()> {
        let mut cursor = cursor.build();
        self.encode_indexed(&mut cursor)?;
        Ok(())
    }
}
...
pub(crate) struct AvroEncoder {
    ...
    /// The workerpool for parallel encoding.
...
Consider defining an index with `CREATE INDEX` and setting `index` field in connector config",
    ...
    fn encode_indexed(&mut self, batch: Arc<dyn SerBatchReader>) -> AnyResult<()> {
        ...
        let factory = Arc::new({
            let tx = tx.clone();
            let value_sql_schema = self.value_sql_schema.clone();
            let value_avro_schema = self.value_avro_schema.clone();
            let key_sql_schema = self.key_sql_schema.clone();
            let key_avro_schema = self.key_avro_schema.clone();
            let skip_schema_id = self.skip_schema_id;
            let update_format = self.update_format.clone();
            let cdc_field = self.cdc_field.clone();
            let value_avro_schema_with_cdc = self.value_avro_schema_with_cdc.clone();
            move || {
                Box::new(AvroParallelEncoder {
                    value_sql_schema: value_sql_schema.clone(),
                    value_avro_schema: value_avro_schema.clone(),
                    key_sql_schema: key_sql_schema.clone(),
        ...
        if self.workerpool.is_none() {
            self.workerpool = Some(Workerpool::new(self.workers)?);
        }

        self.workerpool
            .as_mut()
            .unwrap()
            .process_batch(batch, RecordFormat::Avro, factory)?;

        while let Ok(msg) = rx.recv() {
            match msg {
                ConsumerMessage::BatchStart { step } => self.output_consumer.batch_start(step),
                ConsumerMessage::PushBuffer {
                    buffer,
                    num_records,
                ...
                ConsumerMessage::BatchEnd => self.output_consumer.batch_end(),
            }
        }
