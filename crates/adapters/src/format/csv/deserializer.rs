//! CSV deserializer.
//!
//! This is a copy of the deserialization code from the `csv` crate at
//! <https://github.com/BurntSushi/rust-csv>, particularly `src/deserializer.rs`
//! in that crate.  It is modified to expose the `Deserializer` implementation,
//! because the upstream version does not make it public and we've [failed to
//! convince][1] the maintainer that it's a good idea to do that.  It's not
//! ideal to copy this much code, but the alternative of forking the library
//! entirely seems worse.
//!
//! The upstream code copied from `csv` is dual-licensed under MIT or the
//! UNLICENSE.
//!
//! [1]: https://github.com/BurntSushi/rust-csv/pull/290

// Suppress clippy warnings about stuff that came from upstream.
// This seems better than modifying the code more than necessary.
...
mod tests {
    ...
    fn deserialize_byte_record<'de, D: Deserialize<'de>>(
        record: &'de ByteRecord,
        headers: Option<&'de ByteRecord>,
    ) -> Result<D, ()> {
        let mut deser = byte_record_deserializer(record, headers);

        D::deserialize(&mut deser).map_err(|_| ())
    }

    fn deserialize_string_record<'de, D: Deserialize<'de>>(
        record: &'de StringRecord,
        headers: Option<&'de StringRecord>,
    ) -> Result<D, ()> {
    ...
    fn de<D: DeserializeOwned>(fields: &[&str]) -> Result<D, ()> {
        let record = StringRecord::from(fields);
        deserialize_string_record(&record, None)
    }

    fn de_headers<D: DeserializeOwned>(headers: &[&str], fields: &[&str]) -> Result<D, ()> {
