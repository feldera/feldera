...
impl CircuitThread {
    ...
    fn step(&mut self) -> Result<bool, ControllerError> {
        // Step number under which this step's input is journaled (matches the
        // pre-increment `self.step` used before the journal write moved below).
        ...
        // Journal the step we just executed (FT recording), stamped with the
        // transaction id its circuit evaluation belonged to. Done here, after
        // `step_circuit`, because the transaction id (the engine's per-
        // transaction counter) is only assigned there.
                ...
                if !committed {
...

                    {
                        let mut transaction_info =
                            self.controller.transaction_info.lock().unwrap();
    ...
    fn input_step(&mut self) -> Result<Option<BufferSize>, ControllerError> {
        ...
        // below leave them at these values, so those steps neither journal nor
        ...
        // replay transaction boundary (so the open replay transaction commits
        // before the next transaction's input is fed).
    ...
    /// True while replaying when the next journaled step to feed belongs to a
...
impl ControllerInner {
    ...
    fn replay_open_transaction_id(&self) -> Option<TransactionId> {
        self.transaction_info.lock().unwrap().replay_open_transaction_id
    }
