...
fn repeat_for_budget(name: &str, mut body: impl FnMut()) {
    ...
    info!("{name}: {passes} passes in {:.1}s", start.elapsed().as_secs_f64());
}
...
fn transaction_rounds(
    ...
    let processed =
        || controller.status().global_metrics.num_total_processed_records();
    let end_of_input = || {
        controller
            .status()
            .input_status()
        ...
        for sampler in samplers {
            ...
            regressions.extend(
                samples
                    .windows(2)
                    .filter(|pair| {
                        pair[1].passthrough < pair[0].passthrough
                            || pair[1].counts < pair[0].counts
                    })
    ...
    assert_eq!(final_sample.counts, records as i64, "aggregate view is short");
