...
pub static CHECKPOINT_SYNC_PULL_FAILURE: AtomicU64 = AtomicU64::new(0);
...
pub static CHECKPOINT_SYNC_PUSH_FAILURE: AtomicU64 = AtomicU64::new(0);
...
fn pull_and_gc(
        ...
        .map_err(|e| ControllerError::checkpoint_fetch_error(format!("{e:?}")))
    {
        Err(err) => {
            ...
            CHECKPOINT_SYNC_PULL_FAILURE.fetch_add(1, Ordering::Relaxed);
            return Err(err);
        }
        Ok((cpm, metrics)) => {
            if cpm.uuid != *prev {
