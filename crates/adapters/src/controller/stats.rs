...
/// Reset token.
///
/// A reset token is returned by `POST /output_endpoints/{name}/reset` and
/// identifies the reset generation on a specific endpoint. Pass it to
/// `/reset_status` to check whether the reset has completed.
///
/// The token carries the pipeline's incarnation uuid so that tokens from a
/// previous incarnation (before a crash, suspend, or restart) are rejected:
/// resets do not survive a crash, so clients must reissue them.
#[derive(Debug, ToSchema)]
pub struct ResetToken {
    /// Pipeline incarnation uuid. Used to reject tokens generated by previous
    /// incarnations of the pipeline.
    pub(crate) incarnation: Uuid,

    /// Endpoint id.
    pub(crate) endpoint_id: EndpointId,

    /// Generation bumped by the `request_reset` that produced this token.
    pub(crate) generation: u64,
}

#[derive(Serialize, Deserialize)]
struct ApiResetToken {
    u: Uuid,
    e: EndpointId,
    g: u64,
}
...
impl ResetToken {
    pub fn new(incarnation: Uuid, endpoint_id: EndpointId, generation: u64) -> Self {
        Self {
            incarnation,
            endpoint_id,
            generation,
    ...
    /// Encode token as a URL-safe base64 string.
    pub fn encode(&self) -> String {
        let api = ApiResetToken {
            u: self.incarnation,
            e: self.endpoint_id,
            g: self.generation,
        };
    ...
    pub fn decode(token: &str) -> Result<Self, anyhow::Error> {
        let json = BASE64_URL_SAFE_NO_PAD
            .decode(token)
            .map_err(|e| anyhow!("reset token is not a valid base64 string: {e}"))?;
        match serde_json_path_to_error::from_slice::<ApiResetToken>(&json) {
            Ok(api) => Ok(Self {
                incarnation: api.u,
                endpoint_id: api.e,
                generation: api.g,
            }),
            Err(e) => Err(anyhow!("error parsing reset token: {e}")),
        }
    }
}
...
mod tests {
    ...
    fn reset_token_roundtrip() {
        let original = ResetToken::new(Uuid::new_v4(), 42, 7);
        let decoded = ResetToken::decode(&original.encode()).expect("decode should succeed");
        assert_eq!(decoded.incarnation, original.incarnation);
        assert_eq!(decoded.endpoint_id, original.endpoint_id);
        assert_eq!(decoded.generation, original.generation);
    }

    #[test]
    fn reset_token_rejects_garbage() {
        assert!(ResetToken::decode("!!not-base64!!").is_err());
        // Valid base64 but not a ResetToken JSON shape.
        assert!(ResetToken::decode("aGVsbG8").is_err());
    }
}
