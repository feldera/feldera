...
use feldera_types::runtime_status::{
    ExtendedRuntimeStatus, ExtendedRuntimeStatusError, RuntimeStatus,
};
...
///
/// - A `200 OK` with as body `PipelineStatusResponse`, containing the current pipeline runtime
        ...
        PipelinePhase::Failed(e) => {
            ...
            if matches!(*e, ControllerError::RestoreInProgress) {
                Ok(ExtendedRuntimeStatus {
                    status: RuntimeStatus::Replaying,
                    details: "".to_string(),
                })
