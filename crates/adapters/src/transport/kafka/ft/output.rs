...
use crate::{
    ...
    transport::kafka::DeferredLogging,
};
...
impl OutputEndpoint for KafkaOutputEndpoint {
    ...
    fn push_buffer(&mut self, buffer: &[u8]) -> AnyResult<()> {
        let _guard = span(&self.topic);
        let State::BatchOpen(OutputPosition { transaction, substep }) = self.state else {
            unreachable!(
        ...
        if transaction >= self.next_transaction {
            let key = OutputPosition { transaction, substep };
    ...
    fn batch_start(&mut self, transaction: u64, _batch_type: OutputBatchType) -> AnyResult<()> {
        ...
        if transaction >= self.next_transaction {
            if transaction > self.next_transaction {
                warn!("skipping from transaction {} to {transaction}", self.next_transaction);
            }
        ...
        self.state = State::BatchOpen(OutputPosition { transaction, substep: 0 });
        Ok(())
    }
