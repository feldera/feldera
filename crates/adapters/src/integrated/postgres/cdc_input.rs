...
impl PostgresCdcInputInner {
    ...
    async fn worker_task_inner(
        ...
        // and stored state across restarts. Uses xxh3 (stable across Rust versions)
        // with a normalized connection identity (host:port/db, excludes password).
        let pipeline_id = {
            let identity = format!(
                "{}:{}/{}\0{}\0{}",
                pg_conn.host, pg_conn.port, pg_conn.name,
        ...
        let store = match PostgresStore::new(pipeline_id, pg_conn).await {
            Ok(store) => store,
            Err(e) => {
                let _ = init_status_sender.send(Err(
                    ControllerError::input_transport_error(
