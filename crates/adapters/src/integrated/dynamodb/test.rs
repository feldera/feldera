...
use super::output::{DynamoDBOutputEndpoint, DynamoDBOutputMetrics, DynamoDBWorker};
...
fn empty_endpoint_with_counter(records_written: Arc<AtomicU64>) -> DynamoDBOutputEndpoint {
    DynamoDBOutputEndpoint {
        ...
        handles: Vec::new(),
        records_written,
        metrics: Arc::new(DynamoDBOutputMetrics::default()),
        num_bytes: 0,
        num_rows: 0,
...
#[test]
fn connector_metrics_record_successful_batch_throughput_totals() {
    let records_written = Arc::new(AtomicU64::new(0));
    let mut endpoint = empty_endpoint_with_counter(records_written);

    endpoint.num_bytes = 1234;
    endpoint.num_rows = 7;
    endpoint.consumer().batch_end();

    assert_eq!(
        dynamodb_metric(&endpoint.metrics, "dynamodb_output_batches_written_total"),
        1.0
    );
    assert_eq!(
        dynamodb_metric(&endpoint.metrics, "dynamodb_output_records_written_total"),
        7.0
    );
    assert_eq!(
        dynamodb_metric(&endpoint.metrics, "dynamodb_output_bytes_written_total"),
        1234.0
    );
}
