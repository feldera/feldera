...
impl FieldIdAdapter {
    ...
    fn rewrite_column(
        ...
        // A rewrite of ours may have introduced a column the table schema does
        // not name; it already refers to the file, so leave it alone.
...
mod tests {
    ...
    fn rewrite_first(logical: Vec<Field>, file: Vec<Field>) -> Result<Arc<dyn PhysicalExpr>> {
        let logical = Arc::new(Schema::new(logical));
        let column = Column::new(logical.field(0).name(), 0);
        FieldIdAdapterFactory::new(&[])
            .create(logical, Arc::new(Schema::new(file)))?
