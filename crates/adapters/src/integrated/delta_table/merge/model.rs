...
/// Maintenance interleaved with flushes must never change what a reader sees.
///
/// `compaction_and_reclaim_preserve_a_table_written_in_parallel` runs each maintenance path
/// once, at the end. What it cannot show is a flush that follows one. Both paths rewrite
/// data files under new paths and drop the deletion vectors covering them, and a tombstone
/// addresses a row by path and physical ordinal, so a flush that planned against the old
/// shape would mark rows in a file that no longer holds them -- or that now holds different
/// rows at those offsets. Nothing downstream would notice: the row count stays plausible
/// and the wrong rows go missing.
///
/// So this interleaves flushes, compaction and reclaim at random and compares the table
/// against the oracle after every step, maintenance included. Thread counts vary too, since
/// a parallel flush leaves a different shape for maintenance to work on.
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn maintenance_interleaved_with_flushes_preserves_the_view() {
    use super::compact::compact;
    use super::rewrite::reclaim_superseded_rows;
    use deltalake::{ensure_table_uri, open_table_with_storage_options};

    /// `(data files, files carrying a deletion vector)` in the table's current snapshot.
    fn shape(table: &DeltaTable) -> (usize, usize) {
        let files: Vec<_> = table.snapshot().unwrap().log_data().into_iter().collect();
        let vectored = files
            .iter()
            .filter(|f| f.deletion_vector_descriptor().is_some())
            .count();
        (files.len(), vectored)
    }

    let mut ran = (0usize, 0usize, 0usize);
    // Each path must be seen doing its job, not merely being called: compaction merging
    // files away, and reclaim dropping a vector by rewriting the file under it.
    let (mut compaction_worked, mut reclaim_worked) = (false, false);
    for seed in 0..6u64 {
        let dir = TempDir::new().unwrap();
        let uri = dir.path().to_str().unwrap().to_string();
        let mut table = create_table(&dir).await;
        // Split cheaply, so a round can walk several ranges without a huge fixture.
        let (writer, _) = writer_for_split(&table, 8);
        let mut rng = SmallRng::seed_from_u64(seed);
        let mut model = Model::default();

        for round in 0..14 {
            // Weighted towards flushing: maintenance needs something to maintain, and a
            // table only reaches an interesting shape after several flushes.
            match rng.gen_range(0..100) {
                0..=59 => {
                    let changes = model.next_batch(&mut rng, 40);
                    if changes.is_empty() {
                        continue;
                    }
                    let threads = if rng.gen_bool(0.5) { 4 } else { 1 };
                    apply_attempt_with_threads(&writer, &mut table, &changes, false, threads).await;
                    ran.0 += 1;
                }
                60..=79 => {
                    ...
                    compact(&uri, Default::default(), Duration::from_secs(60))
                        .await
                        .unwrap_or_else(|e| panic!("seed {seed}, round {round}: compact: {e}"));
                    // Compaction commits through its own handle, so ours is a version behind.
                    table = open_table_with_storage_options(
                        ensure_table_uri(&uri).unwrap(),
                        Default::default(),
                    )
                    .await
                    .unwrap();
                    compaction_worked |= shape(&table).0 < before.0;
                    ran.1 += 1;
                }
                _ => {
                    let before = shape(&table);
                    reclaim_superseded_rows(&mut table, Duration::from_secs(60))
                        .await
                        .unwrap_or_else(|e| panic!("seed {seed}, round {round}: reclaim: {e}"));
                    reclaim_worked |= shape(&table).1 < before.1;
                    ran.2 += 1;
                }
            }
            ...
            assert_eq!(
                live_rows(&table).await,
                model.rows,
                "seed {seed}, round {round}: the table diverged from the view"
            );
        }
        assert!(
            !model.rows.is_empty(),
    ...
    // Or the sequence could have skipped a path entirely and still passed.
    let (flushes, compactions, reclaims) = ran;
    assert!(
        flushes > 0 && compactions > 0 && reclaims > 0,
        "every path must have run: {flushes} flush(es), {compactions} compaction(s), \
         {reclaims} reclaim(s)"
    );
    assert!(
        compaction_worked,
        "no compaction ever merged a file away, so the flushes after them proved nothing"
    );
    assert!(
        reclaim_worked,
        "no reclaim ever rewrote a vectored file, so the flushes after them proved nothing"
    );
}
