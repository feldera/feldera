...
impl DeltaTableInputEndpointInner {
    ...
    /// Validate the SQL expressions that apply in every mode. `filter` applies
    /// to follow reads too, so leaving it to the snapshot path fails the
    /// connector on the first commit instead of at startup. The CDC expressions
    /// are validated by [`validate_cdc_config`](Self::validate_cdc_config) and
    ...
    /// The key the Delta log and the file path use for each partition column:
    /// its physical name under column mapping, its logical name otherwise.
    ...
    fn add_partition_columns(
        ...
        for field in self.logical_schema()?.fields() {
            if partition_columns.contains(field.name()) {
                // The log keys partition values by physical name. A missing key
                // means the log did not say; column mapping leaves nothing in
                // the path to fall back on, so this is an error, not a guess.
