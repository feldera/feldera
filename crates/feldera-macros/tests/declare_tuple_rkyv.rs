...
fn rkyv_roundtrip_with_option_sqlstring() {
    ...
    assert!(
        bytes1_none.len() >= core::mem::size_of::<ArchivedTup1<Option<SqlString>>>(),
        "expected archived root to fit, got {} bytes",
        bytes1_none.len()
    );
    ...
    assert!(
        bytes2_none.len() >= core::mem::size_of::<ArchivedTup2<Option<SqlString>, Option<SqlString>>>(),
        "expected archived root to fit, got {} bytes",
        bytes2_none.len()
    );
}
...
fn rkyv_archived_getters() {
    ...
    let t0_none = archived1_none.0.as_ref();
    ...
    let t0_10 = archived2_10.0.as_ref();
    let t1_10 = archived2_10.1.as_ref();
...
fn rkyv_tup12_all_none_size() {
    ...
    assert!(
        bytes.len() >= core::mem::size_of::<rkyv::Archived<Tup12Opt>>(),
        "expected archived root to fit, got {} bytes",
        bytes.len()
    );
