...
// element. The geometry below is the same for every node, so only the image list varies per node, and
// the code chip hangs outside the node box, leaving the node's dimensions - and so the ELK layout -
// untouched. Each distinct image is generated once and shared, so cytoscape decodes it once too.

import { DIAGRAM_PALETTES, type Color, type DiagramTheme } from './diagramTheme.js';
...
/** Cap height as a fraction of the font size, meaning how far a digit or a capital rises above the
 *  baseline. About 0.72 in whatever monospace font the browser resolves. No chip label has a
 *  descender, so this is the whole height of every run drawn below. */
const CAP_HEIGHT_RATIO = 0.72;
/** Y coordinate of the baseline that centers a chip label vertically in a chip `height` px tall. The
 *  label rises `CHIP_FONT_SIZE * CAP_HEIGHT_RATIO` px above the baseline and nothing hangs below it,
 *  so the baseline goes half that far under the chip's midline. */
const baseline = (height: number): number =>
    height / 2 + (CHIP_FONT_SIZE * CAP_HEIGHT_RATIO) / 2;
...
/** The code chip's label, plain and as SVG text takes it. Escaped by hand: no browser API escapes a
 *  string for XML, and this module builds its SVG without a DOM to serialize. The glyph count comes
 *  off the plain form, where each entity is the one character it draws as. */
const CODE_TEXT = '</>';
const CODE_LABEL = '&lt;/&gt;';
const CODE_GLYPHS = CODE_TEXT.length;
...
/** `x` rounded to `decimals` places. The default of two is enough to place a chip to the sub-pixel,
 *  and keeps a 17-digit float out of the URI. */
...
/** The `fd-network` icon of the web-console icon set, drawn as strokes on a 24px grid. */
...
const networkIcon = (x: number, y: number, ink: Color): string =>
