...
/** Reads the numeric value of attribute `name` off the first `<tag>` in `svg`. Matches the name with
 *  its leading whitespace, so `width` does not also match `stroke-width`. */
...
const counter = (leafCount: number, glyph?: Exclude<CounterGlyph, 'count'>) =>
...
describe('counter controls', () => {
    ...
    it('caches each control per palette and width, as it does the counts', () => {
        const expand = (theme: 'light' | 'dark') => nodeChips(false, 7, theme, 'expand')[1]
        expect(expand('dark')).toBe(expand('dark'))
        expect(expand('dark')).not.toBe(expand('light'))
        expect(expand('dark')).not.toBe(nodeChips(false, 7, 'dark', 'collapse')[1])
    })
})

describe('the code chip label', () => {
    it('escapes the angle brackets but counts them as one glyph each', () => {
        // The escaped label is written by hand, so this is what holds it to the glyph count the chip
        // is sized by.
        const code = decode(nodeChips(true, 0, 'light')[0]!)
        expect(code).toContain('>&lt;/&gt;<')
        // Three glyphs, not the nine characters of the escaped form: sizing the chip off the escaped
        // text would make it three times too wide.
        expect(attr(code, 'text', 'textLength')).toBeCloseTo(3 * CHIP_FONT_SIZE * 0.6, 5)
        expect(attr(code, 'svg', 'width')).toBeCloseTo(CODE_CHIP_WIDTH, 5)
    })
})
