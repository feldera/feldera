...
/** Widths already measured, keyed by text. `measureText` is the expensive part of `labelWidth`, and a
 *  diagram measures the same operator names over and over: once per node per build, and again on every
 *  rebuild. The font is a module constant, so an entry never goes stale. */
 ...
 *
 *  Memoized, so a repeated label costs a map lookup. */
export function labelWidth(text: string): number {
    const memoized = measuredWidths.get(text);
    ...
    const width = labelContext === null
        ? Math.ceil(text.length * LABEL_GLYPH_FALLBACK)
        : Math.ceil(labelContext.measureText(text).width);
    measuredWidths.set(text, width);
    return width;
}

/** `z-index` of a traced (colored) edge. Cytoscape draws elements in increasing `z-index` and every
 *  other element here leaves it at the cytoscape default of 0, so any value above 0 puts a traced edge
 *  over every plain one. */
...
/** A color as this diagram writes one: a `#rrggbb` literal, which is what both SVG attributes and
 *  cytoscape styles take. CSS itself offers no type to borrow, and the `#` prefix is as much of the
 *  form as TypeScript can check. */
...
export interface DiagramPalette {
    ...
    navigatorViewport: Color;
    navigatorGraph: Color;
    navigatorInk: Color;
}
