/**
 * The right navigation drawer: on a screen too narrow for the page header, it
 * holds creating a pipeline, opening a support bundle, booking a demo, and the
 * documentation and community links.
 *
 * The layout is rendered whole, so the bundle button opens into the drawer's own
 * dialog host, as it does in the running app.
 */
...
const renderDrawer = async () => {
  ...
  // enough not to need it. The test iframe is narrower than that.
...
describe('(authorized) right drawer', () => {
  ...
  it('offers the support bundle dialog between New Pipeline and Book a demo', async () => {
    ...
    // Centred in the drawer's column, like its neighbours: as wide as its label,
    // with equal space on either side.
