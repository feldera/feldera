 ...
 * until the content above it is scrolled through, and a click on a pinned section
 * scrolls that section into view instead of reaching the controls inside it.
 *
 * The fixture is a scroll container with a tall filler above the group, standing in
...
/** One section, too tall to fit below the middle of the screen. */
 ...
 * second, short section appended to the group. The space below the group gives a
 * short section room to reach the middle of the screen.
 */
...
describe('PinnedSections.svelte', () => {
  ...
  it('shows no more of the group than the peek', async () => {
    ...
    expect(topOnScreen(group)).toBeCloseTo(SCREEN - pinnedPeekHeightPixels, 0)
  ...
  it('lets a click through once the section is in view', async () => {
    ...
    // The section is up, so the click reaches the button inside it.
