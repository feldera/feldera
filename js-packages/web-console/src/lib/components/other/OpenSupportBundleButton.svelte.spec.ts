/**
 * The "Open support bundle" button and the dialog it opens: the remembered
 * bundles, with the picker and "Clear history" in the row below them.
 *
 * The history is the real IndexedDB-backed one; only the tab-opening helpers are
 * mocked, since a test cannot let a new window through. Every assertion measures
 * geometry or the stored state, not class names.
 */
...
// browser permission prompt on screen are stood in for. A stored fake handle
// carries no permission API, a case the real calls answer with "granted"; these
// spies override that answer.
...
// The cache runs for real as well; `rememberSupportBundleFile` is a spy so one
// test can make it decline a file, which is how an archive too big to cache looks
// from here.
...
const seedHistoryOpenedAgo = async (bundles: { name: string; openedMinutesAgo: number }[]) => {
  ...
  for (const { name, openedMinutesAgo } of bundles) {
    await putSupportBundle(undefined, {
      name,
      // Half a minute past the whole minute, so a tick during the test cannot
      // round the rendered value down.
...
describe('OpenSupportBundleButton.svelte', () => {
  ...
  describe('the button', () => {
    it('is outlined in the primary color and says what it opens', async () => {
      ...
      // Outlined, not filled: the filled preset belongs to the dialog's own
      // primary action.
    ...
    it('opens the dialog and nothing else', async () => {
      ...
      // Nothing of the dialog is on screen until the button is clicked, and the
      // button holds no list of its own.
      ...
      const dialog = renderOpenDialog()
      // The history is what the dialog is about, so it is the dialog's title.
      expect(
    ...
    it('takes a class for the caller to place it with, and reports opening', async () => {
      ...
      // callback that closes the drawer once the dialog is up.
      const onOpen = vi.fn()
  ...
  describe('the dialog', () => {
    it('puts the picker in the bottom left corner, below the bundles', async () => {
      ...
      // Below the bundles, sharing its row with "Clear history" and sitting left of
      // it.
    ...
    it('says how long ago each bundle was opened', async () => {
      ...
      // Days, hours and minutes, as the pipelines table reports a status age.
    ...
    it('puts the elapsed time at the right edge of the row', async () => {
      ...
      // After the name and hard against the row's padding, so the times line up
      // down the list whatever the names are.
    ...
    it('cuts a name off with an ellipsis and keeps the list inside the screen', async () => {
      ...
      // The name is clipped to the room it has, with the whole of it in the title.
      expect(getComputedStyle(name).textOverflow).toBe('ellipsis')
      expect(name.scrollWidth).toBeGreaterThan(name.clientWidth)
      expect(long.title).toBe(longName)
      // Nothing widens the list, and the list does not outgrow the screen: no
      // sideways scrolling anywhere.
      ...
      // The elapsed time is what the name gives way to, and it keeps its place.
  ...
  describe('opening a remembered bundle', () => {
    it('opens it in a new page and makes it the most recent', async () => {
      ...
      // closes behind the tab it opened.
    ...
    it('asks for access and goes straight to the profile', async () => {
      ...
      // The prompt comes first, then the profile opens: nothing else stands between
    ...
    it('leaves the bundle unopened when access is refused', async () => {
      ...
      expect(goto).not.toHaveBeenCalled()
      // The dialog stays put, so the refusal can be answered with another try.
      expect(useGlobalDialog().dialog).not.toBe(null)
    })
  })
  ...
  describe('picking a bundle from disk', () => {
    it('picks straight from the button and confirms in the popup', async () => {
      ...
      // The confirmation opens upwards, since the button it hangs from sits at the
      // bottom edge of the dialog: below it, the confirmation would be off screen.
      const pick = dialog.querySelector<HTMLElement>('[data-testid=btn-pick-support-bundle]')!
      ...
      // Picking remembers the bundle, so the list shows it and the viewer is
      ...
      // The viewer re-reads the file itself: no bytes are handed over, and the
      // dialog closes.
      expect(openStoredBundleTab).toHaveBeenCalledWith(id)
    ...
    it('remembers a bundle the file input handed over', async () => {
      ...
      // The viewer reads the copy, so nothing is handed over and the history
      // keeps the bundle for next time.
