// CurrentTenant drives from the session's membership list: the acting tenant
// name always renders, the dropdown affordance appears only when there is more
// than one membership, and switching selects by tenant id.
...
describe('CurrentTenant', () => {
  ...
  it('renders nothing when there is no tenant name and no choice to make', async () => {
    felderaState.current = { tenantId: '', tenantName: '', memberships: [] }
    await render(CurrentTenant)
    expect(document.body.textContent).not.toContain('Tenant')
  })
  ...
  it('shows the plain acting tenant name for an owner with no memberships', async () => {
    felderaState.current = { tenantId: 't-x', tenantName: 'acted-as', memberships: [] }
    await render(CurrentTenant)
    expect(selectElement()).toBeNull()
