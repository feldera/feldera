 ...
 *
 * An entry remembers the archive one of two ways, and IndexedDB is what makes
 * either possible: both a `FileSystemFileHandle` and a `File` are
 * structured-cloneable but not JSON-serializable, so `localStorage` can hold
 * neither.
 *
...
type SupportBundleFacts = {
  ...
  id: number
  /** File name, as shown on the tiles and in the bundle list. */
  name: string
  /** Epoch ms of the last time the bundle was opened; the list is sorted by it. */
  openedAt: number
}
  ...
  new Promise<IDBDatabase>((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      ...
      if (!db.objectStoreNames.contains(STORE_NAME)) {
        // Keys are generated: the id is an opaque handle for URLs, nothing
        // about the file can serve as a stable primary key.
 ...
 *
 * `use` must issue all of its IndexedDB requests before awaiting anything else:
 * a transaction commits as soon as the browser's event loop finds it idle, so
 * awaiting an unrelated promise in the middle would close it. Callers that need
 * to interleave other async work therefore split it into several transactions.
 */
...
export const listSupportBundles = async (): Promise<StoredSupportBundle[]> => {
  ...
  // Ties (two bundles opened within the same millisecond) fall back to the key,
  // so the order is at least the order they were stored in.
...
const findSameEntry = async (
  ...
  for (const bundle of bundles) {
    ...
    // A handle read back from IndexedDB comes from whatever browser stored it,
    // so `isSameEntry` is treated as optional here.
...
/**
 * Writes an entry as the most recent one. Shared with `supportBundleCache`, which
 * adds its own pruning pass; a caller here is responsible for pruning.
 */
...
/**
 * Records a picked bundle as the most recent one. Re-picking a file already in
 * the history moves that entry to the front instead of duplicating it.
 *
 * Only the count is pruned afterwards: a handle takes no space to speak of, so
 * remembering one cannot put the cache over its byte budget.
 */
...
/**
 * Whether the bundle can be read without asking the user again. Browsers drop
 * file grants between sessions, so a handle that was picked in an earlier visit
 * reports 'prompt' here. A cached copy belongs to this origin and needs nobody's
 * permission.
 */
...
export const readStoredBundle = async (bundle: StoredSupportBundle): Promise<Uint8Array> => {
  ...
  // Unreachable through this module, but the records come out of IndexedDB
  // untyped: an entry holding neither a handle nor a copy is reported rather than
  // crashed on.
