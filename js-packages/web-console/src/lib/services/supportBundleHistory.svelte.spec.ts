/**
 * The IndexedDB-backed support bundle history. Runs in the browser project
 * because it drives a real IndexedDB and real structured clone: those are what
 * decide whether a File System Access handle can be stored at all. The other kind
 * of entry, a cached archive, is covered by `supportBundleCache.svelte.spec.ts`.
 */
...
describe('supportBundleHistory', () => {
  ...
  describe('remembering bundles', () => {
    ...
    it('keeps the handle usable across a database round trip', async () => {
      ...
      // Nothing about the entry survives if the handle cannot be cloned, which
      // is what rules out storing bundles in localStorage.
  ...
  describe('reading a bundle', () => {
    ...
    it('reports an entry that says nothing about where to read', async () => {
      // Unreachable through this module; the records come out of IndexedDB
      // untyped, so a database written by other means is reported, not crashed on.
  ...
  describe('read permission', () => {
    ...
    it('treats a browser without the permission API as granting access', async () => {
      // Reading either works or throws there; reporting 'granted' lets the
      // caller find out which instead of blocking on a prompt it cannot show.
