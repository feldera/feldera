...
Create a new Databricks notebook and connect it to a cluster.
...
Next, we query these views to simulate a batch job by querying all of these
views:
...
- We defined a simple batch job using Spark in Databricks.
- We simulatesd the batch job by executing all the queries, which
  took approximately **41 seconds**.
- The batch job needs to ran periodically to process updates to the input
  Delta table.
