...
**always-on**, incremental Feldera pipeline **without modifying the queries**.

To acheive this, we will:

- Similar to the Spark job in the previous article, we configure Feldera to
  read from Delta Lake.
- Use Feldera Ad-Hoc queries to query these views.
- Demonstrate incremental computation by adding new records.
...
Now we will demonstrate incremental computation by adding a new `LINEITEM`.

The following ad-hoc query will insert a new `LINEITEM`, resulting in a new row
in the output of the query `q1`:
...
Once this new record is inserted, we can **immediately** view the output of the
query `q1`:
...
As we can see in the table above, there is a new row that didn't exist
previously, without needing to re-run the pipeline.

## Takeaways

- We converted the Spark batch job into an **always-on**, incremental pipeline.
- We demonstrated incremental computation by adding new record and **instantly**
  querying the view, without needing to re-run the pipeline.
