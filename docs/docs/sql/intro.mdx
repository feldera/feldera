...
## Feldera Analytics Platform, DBSP and incremental view maintenance

Feldera Analytics Platform's implementation is based on a query engine optimized
for *incremental view maintenance*. This engine is called Database Stream
Processor, DBSP.

Feldera Platform is used in the following way:

- users define a set of database tables in the Feldera Platform. These tables
