...
| `ARRAYS_OVERLAP(` _array1_ `, ` _array2_ `)`                         | Returns `NULL` if _array1_ or _array2_ is `NULL`. Returns true if the two arrays have at least one element in common. Returns `NULL` if either of the arrays have a `NULL` element and no common element. Returns false otherwise.        | `ARRAYS_OVERLAP(ARRAY [1, 2, 3], ARRAY [3, 4, 5])` => true          |
