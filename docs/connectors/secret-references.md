...
as files at: `/etc/feldera-secrets/<name>/<data key>`.
...
- Most string values in the connector configuration JSON can be identified to be secret references
  (this excludes keys), for example (secret named `a` at data key `b` has value `value`):
  ```
  {
    "${secret:kubernetes:a/b}": "${secret:kubernetes:a/b}",
    "a1": "${secret:kubernetes:a/b}",
    "a2": [ "${secret:kubernetes:a/b}" ]
  }
  ```
  ... will be resolved to:
  ```
