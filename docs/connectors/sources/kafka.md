...
| `kafka_options` (required) | map              |         | Key-value pairs for configuring `librdkafka`. At least `bootstrap.servers` must be specified. |
...
The connector passes `kafka_options` directly to [**librdkafka**](https://github.com/confluentinc/librdkafka/blob/master/CONFIGURATION.md).  Some of the relevant options:
