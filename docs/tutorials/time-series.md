...
This guide covers concepts required to efficiently process **time series**
...
classes of queries faster and/or using less memory over time series:

  * It takes advantage of the fact that timestamps in a time series grow monotonically or
    almost monotonically to discard old data no longer needed to compute outputs
    for new data points. In order to perform these optimizations the query engine needs
    to know which columns in the input time series grow monotonically.  This requires
  ...
  * Time series data is often append-only: once an event has been appended to the
    ...
    can provide this information to the engine using [`append_only` annotatuons](#append-only-tables).
...
* **Inputs that violate lateness are discarded.**  When a program receives a record
  ...
  previously observed in the same column, the pipeline can drop such a record.
...
Consider the following table declaration with 1-hour lateness on the `ts` column:
...
```sql
INSERT INTO purchase VALUES('2020-01-01 00:00:00', 10);

INSERT INTO purchase VALUES('2020-01-01 01:00:00', 20);

-- Late row, but within the lateness bound.
INSERT INTO purchase VALUES('2020-01-01 00:10:00', 15);

INSERT INTO purchase VALUES('2020-01-01 02:00:00', 65);

-- Late row that violates lateness.
INSERT INTO purchase VALUES('2020-01-01 00:20:00', 12);
```
...
materialized tables and views.  See [documentation](/sql/materialized) for details.
...
the query engine would assume that any record in this table can be inserted or
deleted at any time.  Deleting an old record requires re-computing
the daily maximum for the corresponding date.  This in turn requires storing
the entire purchase history permanently.

**We can do better with `LATENESS`**.  The `LATENESS` annotation guarantees
that changes to the table cannot go more than one hour back.  Hence, daily
maxima can only change for the current and, possibly, previous day.  Since
we will never need to reevaluate the query for earlier dates, we do not need
to store older data.
...
Feldera uses the following (simplified) procedure to identify and discard unused
data:

1. **Compute waterlines of all program relations** through a combination of offline
   and online dataflow analysis:
...
3. **Discard old records**. Feldera garbage collects old records that fall below retention
   bounds in the background.  Since these records are guaranteed to have no effect on
   new outputs, it is safe to keep them around until a convenient time when they can be
   removed with low overhead.  In practice, we incorporate garbage collection in the
   [log-structured merge tree](https://www.feldera.com/blog/feldera-storage)(LSM) maintenance process.

**Not all SQL operators allow discarding old inputs**. Joins, aggregates, and `DISTINCT`'s
for example may, depending on their exact join condition or the `GROUP BY` clause,
require storing their entire inputs permanently.  We list operators for
which Feldera implements garbage collection [below](#sql-for-time-series-analysis).
...
* **The application is only allowed to act on the final
    ...
    output is final. Handling intermediate outputs can
...
|-------------------------------------|----------------------------|------------------------------|------------------------------|-------------------------------------------------|-----------------------------|
| `VALUES('2020-01-01 01:00:00', 10)` | '2020-01-01 01:00:00'      | '2020-01-01 00:00:00'        | '2020-01-01 00:00:00'        | `insert: {"d":"2020-01-01 00:00:00","total":10}`|                             |
| `VALUES('2020-01-01 02:00:00', 10)` | '2020-01-01 02:00:00'      | '2020-01-01 01:00:00'        | '2020-01-01 00:00:00'        | `delete: {"d":"2020-01-01 00:00:00","total":10}`|                             |
|                                     |                            |                              |                              | `insert: {"d":"2020-01-01 00:00:00","total":20}`|                             |
| `VALUES('2020-01-02 00:00:00', 10)` | '2020-01-02 00:00:00'      | '2020-01-01 23:00:00'        | '2020-01-01 00:00:00'        | `insert: {"d":"2020-01-02 00:00:00","total":10}`|                             |
| `VALUES('2020-01-02 01:00:00', 10)` | '2020-01-02 01:00:00'      | '2020-01-02 00:00:00'        | '2020-01-02 00:00:00'        | `delete: {"d":"2020-01-02 00:00:00","total":10}`| `insert: {"d":"2020-01-01 00:00:00","total":20}`|
...
CREATE TABLE order_pickup (
   pickup_time TIMESTAMP NOT NULL WATERMARK INTERVAL 1 HOURS,
   location VARCHAR
);
```
...
```sql
CREATE VIEW rolling_daily_max AS
SELECT
    ts,
    amount,
...
Feldera features an efficient incremental implementation of this operator
...
```sql
-- All purchases in the last week
