...
Deletions are recomputed incrementally as well. Ad-hoc queries do not support
`DELETE`; deletions reach the pipeline through [input connectors](/connectors/)
or the [REST API](/api/insert-data), encoded in the
[`insert_delete` JSON format](/formats/json). The following request deletes the
edge (0, 1) from `EDGES`:

```bash
curl -X POST 'http://127.0.0.1:8080/v0/pipelines/{pipeline_name}/ingress/EDGES?format=json' \
  -d '{"delete": {"x": 0, "y": 1}}'
```

`CLOSURE` now contains only the edges reachable without the deleted edge:
...
To debug, you can use a [UDF](/sql/udf). For example, modify the previous example to use a Rust function that
