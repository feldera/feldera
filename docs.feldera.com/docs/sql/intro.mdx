...
- A traditional database offers durable storage of data.  In general,
  DBSP does not store the contents of the data, for either the tables
  or the defined views.  DBSP will only store sufficient data to compute
...
- Other traditional database constructs, such as triggers, do not
  make sense for a system like DBSP.
