...
| <a id="array_exists"></a>`ARRAY_EXISTS(` _array_, _predicate_ `)` | The _predicate_ is a lambda expression with an argument that has the array element type, returning a Boolean. `ARRAY_EXISTS` returns `true` when the lambda returns `true` for at least on element. If the array is empty, returns `false`. If the result is not true and the lambda evaluates to NULL for any element, it returns NULL.                                | `ARRAY_EXISTS(ARRAY[1, -2, 3], x > 0)` => true                            |
