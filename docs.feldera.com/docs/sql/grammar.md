...
or `CHAR` and is a hint to the compiler to use an interned
...
string in memory and replace such strings with a cryptographic hash
function of the string value.  This work well if strings are only used
for equality comparisons; when interned strings participate in other
