...
| `skip_unused_columns` (<b>DEPRECATED</b>) | bool   | false | <p>This connector-level property is deprecated. Use the [table-level `skip_unused_columns` property](/sql/grammar#ignoring-unused-columns) instead.</p><p>Don't read unused columns from the Delta table. When set to `true`, this legacy option instructs the connector to avoid reading columns from the Delta table that are not used in any view definitions. With this deprecated connector-level option, columns must be either nullable or have default values to be skipped. This can improve ingestion performance, especially for wide tables.</p><p>Note: The simplest way to exclude unused columns is to omit them from the Feldera SQL table declaration. The connector never reads columns that aren't declared in the SQL schema. Additionally, the SQL compiler emits warnings for declared but unused columns—use these as a guide to optimize your schema.</p>|
