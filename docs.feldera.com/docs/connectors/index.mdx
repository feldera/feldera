...
* `paused` - If set to true the connector is created in the Paused state:
   an input connector does not fetch data from its source, and an output
   connector discards the output of its view instead of writing it to its sink.
   The connector stays in that state until it is started through the API
   ([input](/api/control-input-connector), [output](/api/control-output-connector)).
   See [connector orchestration](/connectors/orchestration).
...
* `max_output_buffer_time` - Maximum time data is kept in the output
   buffer, for example `"500ms"` or `"10s"`. Replaces the deprecated
   `max_output_buffer_time_millis` (integer milliseconds).
...
* [Connector orchestration](/connectors/orchestration)
