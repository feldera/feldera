...
Read the polarity of a record with the
...
A table fed this way is an append-only log of changes. A SQL query can be used
...
reports for each message as the `kafka_timestamp` metadata attribute:
...
-- Every change the stream reported, deletions included.
CREATE MATERIALIZED VIEW history AS SELECT * FROM changes;
...
Changes of the same key that share a timestamp rank in an arbitrary order,
which is why the query breaks the tie on `is_delete`. A CDC stream reports an
update as a single message that deletes the old value of a record and inserts
the new one, so those two changes carry one timestamp; ranking the insertion
first keeps the updated record, where ranking the deletion first would drop it.

Notes and restrictions:

* The table must not have a primary key.
...
* Deletions that a soft-delete connector reads do not cancel out earlier
  insertions of the same record, so the table can hold several copies of a
