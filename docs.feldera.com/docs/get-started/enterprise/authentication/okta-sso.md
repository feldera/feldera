...
This guide explains how to configure Okta as an authentication provider for Feldera Enterprise, enabling seamless SSO integration for B2B deployments.
...
- **Maintain security compliance** through enterprise-grade identity management
...
### 5. Create Custom Authorization Server
...
Feldera validates all JWT tokens using:
- **Signature verification** using Okta's public keys (JWK)
- **Expiration time** validation (`exp` claim)
- **Issuer validation** (`iss` claim against configured issuer)
- **Audience validation** (`aud` claim against client ID)

### Tenant Isolation

- **Complete isolation** between tenants at the database level
- **API access control** ensures users can only access their tenant's resources
- **Pipeline isolation** prevents cross-tenant data access

### Best Practices

1. **Use HTTPS** for all Feldera deployments with authentication
2. **Configure appropriate token lifetimes** in Okta (1 hour recommended)
3. **Regularly rotate client secrets** if using client credentials flow
4. **Monitor authentication logs** for security events
5. **Use tenant claims** for fine-grained access control in enterprise deployments
...
1. **Check Okta application configuration**:
   - Verify redirect URLs include trailing slash
   - Confirm grant types are enabled
...
3. **Check Feldera logs** for authentication errors:
   ```bash
   kubectl logs deployment/pipeline-manager | grep -i auth
   ```

4. **Test token manually** using Okta's token introspection endpoint

## Next Steps

After configuring Okta authentication:

1. **Test the login flow** with a test user
3. **Configure user groups** for appropriate access patterns
4. **Set up monitoring** for authentication events
5. **Train users** on the new SSO login process
