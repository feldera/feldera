...
you configured. A tenant also records the issuer it was first seen under, but
that is provenance only: it does not take part in identifying the tenant, so
changing the issuer never creates a second tenant of the same name.
...
The practical consequence is worth stating plainly: a tenant can survive a
provider change, but the users in it cannot. They arrive as new identities and
must be granted their roles again.
...
strategy, never to `default`, so pipelines and API keys created beforehand are
not visible to ordinary logins. The data is intact, just in a tenant nobody
...
Recreate the resources in a regular tenant from there. Plan this before enabling
...
expect to change providers, prefer it.
