    ...
    return systemMessages.filter((message) => {
      return match(message.dismissable)
        ...
        .with('session', () => {
          ...
          const startOfTheDayTimestamp = ((now) => now - (now % 1000) * 3600 * 24)(dateNow())
          return (
            !shownTimestamp ||
        ...
        .with({ forMs: P.select() }, (periodMs) => {
          const shownTimestamp = shownMessage(message.id)?.timestamp
          return !shownTimestamp || shownTimestamp <= dateNow() - periodMs
        })
        .exhaustive()
    })
  })
  ...
  return {
    ...
    dismiss(id: string) {
      ...
      shownMessages.value = nubLast(
        [{ id, timestamp: dateNow() }, ...shownMessages.value],
        (shown) => shown.id
      )
    },
    /**
