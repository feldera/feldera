...
export const getLicenseMessage = (config: Configuration, now: Date): SystemMessage | null => {
  {
    ...
    if (license) {
      if (new Date(license.valid_from) > now) {
        // License activates later
        const timeIn = `{toDaysHoursFromNow ${new Date(license.valid_from).valueOf()}}`
        return {
          id: `license_${config.edition}_future`,
          dismissable: 'never',
          text: license.is_trial
            ? `Your trial license will become active in ${timeIn}`
            : `Your Feldera ${config.edition} license will become active in ${timeIn}`
        }
      } else if (
        license.valid_until &&
