...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="run the filing and report it, without writing the changelog",
    )
    parser.add_argument(
        "--tag",
        required=True,
        help="tag of the published release, for example v0.348.0",
    )
    ...
    try:
        ...
        filing = file_release(current, at_tag, version)
    except ChangelogError as error:
        print(f"::error file={args.changelog}::{error}", file=sys.stderr)
        return 1

    if filing.text == current.text:
        print(f"{args.changelog}: nothing to file under {version}")
        return 0
    if args.dry_run:
        print(
            f"{args.changelog}: would file {filing.filed} entries under {version}, "
            f"{filing.pending} would stay unreleased"
        )
        return 0
    written = args.changelog.with_name(args.changelog.name + ".new")
    written.write_text(filing.text)
    os.replace(written, args.changelog)
    print(
        f"{args.changelog}: filed {filing.filed} entries under {version}, "
        f"{filing.pending} stay unreleased"
    )
    return 0
