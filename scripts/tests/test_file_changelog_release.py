...
import file_changelog_release  # noqa: E402
...
class DryRunTest(unittest.TestCase):
    """The merge queue runs the filing with --dry-run, which must report and not write."""

    def run_filing(self, current, at_tag):
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "changelog.md"
            path.write_text(current)
            with (
                mock.patch.object(
                    file_changelog_release, "changelog_at_tag", return_value=at_tag
                ),
                contextlib.redirect_stdout(io.StringIO()),
                contextlib.redirect_stderr(io.StringIO()),
            ):
                status = main(
                    ["--dry-run", "--tag", "v0.348.0", "--changelog", str(path)]
                )
            return status, path.read_text()

    def test_reports_the_filing_and_leaves_the_changelog_alone(self):
        current = changelog("- merged after the release\n\n- shipped one")

        status, after = self.run_filing(current, changelog("- shipped one"))

        self.assertEqual(status, 0)
        self.assertEqual(after, current)

    def test_reports_a_changelog_the_filing_cannot_read(self):
        current = changelog("- an entry").replace("## Unreleased", "## Next")

        status, _ = self.run_filing(current, changelog("- an entry"))
