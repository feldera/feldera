...
import { gcd, fail, enforceNumber, Option, OMap, type Comparable, assert, NumericRange } from "./util.js";
...
class PercentValue extends PropertyValue {
    ...
    plus(other: PropertyValue): PropertyValue {
        if (other instanceof PercentValue) {
            let denominator = gcd(this.denominator, other.denominator);
            let scale = denominator / this.denominator;
            let otherScale = denominator / other.denominator;
            let numerator = this.numerator * scale + other.numerator * otherScale;
