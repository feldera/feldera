...
import { SourcePositionRanges, SourcePositionRange, Sources, type Dataflow } from "./dataflow.js";
...
export class CircuitProfile {
    ...
    setDataflow(dataflow: Dataflow) {
        ...
        for (const [_, info] of Object.entries(mir)) {
            let persistent_id = info.persistent_id;
            if (!persistent_id) continue;
            let profileNode = this.byPersistentId.get(persistent_id);
            if (profileNode.isNone())
                continue;
            let n = profileNode.unwrap();
            if (info.table !== undefined) {
                n.operation += " " + info.table;
            } else if (info.view !== undefined) {
                n.operation += " " + info.view;
            }

            n.setSourcePositions(new SourcePositionRanges(
                info.positions.map(p => new SourcePositionRange(p))));
        }
    }
    ...
    addEdge(e: JsonCircuitEdge) {
        ...
        if (source.isSome() && target.isSome()) {
            let sourceLabel = source.unwrap().id;
            let targetLabel = target.unwrap().id;
