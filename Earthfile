...
build-kafka-connect-container:
    FROM DOCKERFILE -f deploy/Dockerfile.extra --target kafka-connect .
...
test-sec-ops:
    ...
    WITH DOCKER --pull redpandadata/redpanda:v23.3.21 \
        ...
        RUN COMPOSE_HTTP_TIMEOUT=120 RUST_LOG=debug,tokio_postgres=info docker-compose \
            -f docker-compose.yml -f docker-compose-extra.yml -f docker-compose-demo.yml \
            --profile demo-sec-ops up --force-recreate --exit-code-from demo-sec-ops
    END
...
ci-tests:
    ...
    # BUILD +test-s3
    # TODO: after it has been tested in CI, bring the below back to nightly only
    # nightly-tests:
